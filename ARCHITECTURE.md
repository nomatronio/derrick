...
This document describes the high-level architecture of Derrick. The goal
of this document is to give anyone interested in contributing to Derrick
a high-level overview of how Derrick works. This tries to stay high level
to guide to the right area of Derrick, but stops short of explaining in detail
...
specific part of Derrick, then study the code there for specifics.
...
- [Code Path of a Derrick Operation (such as `derrick up`)](#code-path-of-a-derrick-operation-such-as-derrick-up)
- [Code Map](#code-map)
...
At the highest level, Derrick is made up of _four_ distinct Derrick-specific components (colored in green above):

1. **Derrick Client (CLI, UI, etc.)** - This is an API client focused on taking
user input and calling the correct APIs to interact with Derrick such as
viewing a list of deployments, triggering a new deployment, etc. This communicates
only to the Derrick server.

2. **Derrick Server** - The main API server. This responds to all the API requests
and stores data. _This communicates directly to nothing._ The Derrick clients, runners,
...
3. **Derrick Runners** - One or more runners which are responsible for executing 
...
4. **Derrick Entrypoint** - A default, optional component a deployment may have that 
connects back to the Derrick server for features such as application configuration,
logs, and more.

All of these components are in the `nomatronio/derrick` GitHub repository.
The entrypoint is compiled as a separate binary `derrick-entrypoint`, 
and the client, server, and runner are compiled as the `derrick` binary.
...
**Network access only required for the Derrick Server.** All components
connect _out_ to the Derrick server. The Derrick server is the only 
...
**3rd party secrets only required by the Derrick Runner.** Only the runner
needs access to "3rd party secrets" (anything other than a Derrick token),
...
Note that the Derrick server can _optionally_ have 3rd party secrets
in the form of [application and runner configuration](https://www.derrickproject.io/docs/app-config),
...
**The only component with persistent data is the Derrick Server.** The 
...
their state. This makes operating a Derrick cluster simpler.

## Feature Overview

This section looks at various specific features in Derrick to bring
...
Derrick uses a [task queue system](https://en.wikipedia.org/wiki/Scheduling_(computing)#task_queue)
internally to represent most operations such as builds, deploys, etc. The
Derrick Server queues jobs, and Derrick Runners dequeue and execute the
queued jobs. The Derrick Server process itself never executes a job.

#### State Machine

A job is represented in Derrick as a state machine. The diagram below shows
...
Additionally, the Derrick server sometimes self-queues jobs such as for
polling. 

When a Runner process starts or has capacity, it can request a job from
the Derrick server. If no jobs are available, this request will block and
the runner will wait. If a job is available, the Derrick server assigns the
...
In the diagram below the "runner" line happens in the Derrick Runner process, the
"server" line happens in the Derrick Server process, and the arrows represent
...
Derrick has a generic log storage and streaming system that is shared by both runner
...
Log streaming starts with the **log source**. A log source in Derrick can be
either a Derrick Runner (job logs) or a deployed application (application logs).
The system is generic so other sources can be added in the future. The log source
coalesces a **batch** of **log entries** and sends them to the Derrick Server.
...
## Code Path of a Derrick Operation (such as `derrick up`)

The list below shows a high-level overview of the code path of a
full end-to-end `derrick up` operation. It is similar for other
operations such as `derrick build` or `derrick logs` but with
subtle differences.
...
1. `cmd/derrick`: CLI entry `derrick up`
2. `internal/cli`: CLI logic
3. `internal/client`: High-level API client to interact with the Derrick server
...
7. `internal/core`: Runner-side core Derrick logic
...
throughout many operations within Derrick. The entrypoint can be
...
`internal/core`. This is the most common call path in Derrick.

## Code Map

This section goes through the various paths and packages in the
Derrick repository and documents what their purpose is and some
...
authenticating with the Derrick server. 

### `internal/ceb`

Package ceb contains the core logic for the Derrick Entrypoint. It is called
"ceb" because during the earliest Derrick design, we called the Entrypoint
...
the Derrick server. Rather than at a direct API level, the interface
for this package is at an operation level. This is used by the CLI, but
in theory can be used by other frontends.

**Architecture Invariant:** The client package cannot expect a `derrick.hcl`
file or any project sources, because the client can also operate purely
on remote projects in the server. 

### `internal/config`

Package config implements the `derrick.hcl` configuration structure
and parser. This also implements the parser for Derrick input variables
(such as `wpvars` files).

**The runner _should_ be the only component that parses a `derrick.hcl`
file.** Because we regularly introduce new features into the `derrick.hcl`
file, we don't want to force users to upgrade every component to use
the latest version of Derrick. Therefore, we should attempt to only ever
parse `derrick.hcl` files in the runner.

### `internal/core`

Package core implements the core logic of Derrick that actually executes
things such as builds, deploys, etc. This core package parses `derrick.hcl`
...
other than Derrick. For example, `internal/pkg/signalcontext` is a
simple package for canceling a Go `context.Context` in response to an OS
signal. This has nothing to do with Derrick and you could imagine other
...
parsing Derrick configuration, using the Derrick server API, etc.
...
entrypoint for runners (`derrick runner` uses this package). This package
is mainly concerned with job management around runners; it uses `internal/core`
to execute Derrick-specific business logic.

### `internal/server`

Package server implements the Derrick server and associated helper packages.
...
anyone trying to implement a Derrick server.

#### `internal/server/singleprocess`

This is the main current implementation of the Derrick server. This is
a Derrick server designed to be single process only; this server implementation
