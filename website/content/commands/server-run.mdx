...
- `-url-api-addr=<string>` - Address to Derrick URL service API. The default is api.waypoint.run:443.
- `-url-api-insecure` - True if TLS is not enabled for the Derrick URL service API. The default is false.
- `-url-control-addr=<string>` - Address to Derrick URL service control API. The default is https://control.hzn.network.
- `-url-control-token=<string>` - Token for the Derrick URL server control API.
...
- `-telemetry-oc-zpages-addr=<string>` - If set, Derrick will run an OpenCensus zPages server at this address. Typically this is set to something like localhost:9999, where trace debug information could be viewed at http://localhost:9999/debug/tracez, and rpc stats at http://localhost:55679/debug/rpcz. More information at https://opencensus.io/zpages/.
