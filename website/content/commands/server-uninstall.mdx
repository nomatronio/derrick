...
description: 'Uninstall the Derrick server'
---

# Derrick Server uninstall

Command: `derrick server uninstall`

Uninstall the Derrick server
...
- `-platform=<string>` - Platform to uninstall the Derrick server from.
...
- `-snapshot` - Enable or disable taking a snapshot of Derrick server prior to uninstall. The default is true.
...
- `-ecs-cluster=<string>` - Configures the Cluster to uninstall. The default is derrick-server.
- `-ecs-region=<string>` - Configures which AWS region to uninstall from.

#### kubernetes Options

- `-k8s-context=<string>` - The Kubernetes context to unisntall the Derrick server from. If left unset, Derrick will use the current Kubernetes context.
- `-k8s-namespace=<string>` - Namespace in Kubernetes to uninstall the Derrick server from. The default is default.
