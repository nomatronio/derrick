...
app "wpmini" {
  ...
  build {
    ...
    registry {
      use "docker" {
        image = "gcr.io/derrick-project-id/wpmini"
        tag   = "latest"
      }
    }
  }
  ...
  deploy {
    use "google-cloud-run" {
	  ...
	  service_account_name = "cloudrun@derrick-project-id.iam.gserviceaccount.com"

      auto_scaling {
        max = 10
      }

      cloudsql_instances = ["derrick-project-id:europe-north1:sql-instance"]
