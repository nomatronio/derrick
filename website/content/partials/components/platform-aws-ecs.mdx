...
List of existing group IDs to use for the ECS service's network access. If none are specified, derrick will create one. If DisableALB is false (the default), derrick will only allow ingress from the ALB's security group.
