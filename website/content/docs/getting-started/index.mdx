---
layout: docs
page_title: Getting Started with Derrick
description: |-
  Install and run Derrick with this five minute quick start tutorial.
---

# Getting Started with Derrick

It only takes a few minutes to get started with Derrick on your local
...
Windows and macOS. This will introduce you to the basics of Derrick, even
if your target environment is not Docker.

Afterwards, go in-depth with [tutorials](/derrick/docs/) for AWS, Azure, GCP, Kubernetes, and
Nomad. For detailed
Kubernetes-focused documentation, see the
[dedicated Kubernetes section](/derrick/docs/platforms/kubernetes/install).

-> **Note**: We also have Derrick Getting Started tutorials for other platforms.
Check out the Getting Started tutorials for your favorite platform.

- [Getting Started with Nomad](/derrick/docs/get-started-nomad)
- [Getting Started with Kubernetes](/derrick/docs/get-started-kubernetes)
- [Getting Started with Docker](/derrick/docs/get-started-docker)

## Installing Derrick

[Download](/derrick/downloads) the Derrick binary and install it to your system. The
...
Install the Derrick server to Docker with the `install` command. The
`-accept-tos` flag is required to use the `derrick.run` URL publishing service.

```shell-session
$ derrick install -platform=docker -accept-tos
```
...
Advertise Address: derrick-server:9701
HTTP UI Address: localhost:9702
```

## Initializing Derrick

Clone the git repository with Derrick application examples, or [download the
code](https://github.com/nomatronio/derrick-examples) directly from GitHub.
...
```shell-session
$ cd derrick-examples/docker/nodejs
```

All the code you need is provided in this directory, including a `derrick.hcl`
...
Now, run `derrick up` to build, deploy, and release the application.
...
Derrick includes several sub-commands to help you monitor and debug your application.

Run `logs` to view Derrick's logs. This may not include all of your
application-specific logs, but will display Derrick's logs.
...
Derrick includes a web interface for viewing logs, deployments, builds, and
...
```shell-session
$ derrick destroy -auto-approve
```

## Next Steps

You have now used Derrick to build, deploy, and release a NodeJS application to Docker.
Derrick removed the overhead of managing different technologies to build, deploy, and release applications.

Check out the other in-depth tutorials for AWS, Azure, GCP, Kubernetes, and
Nomad.

- [Azure](/derrick/docs/deploy-azure)
- [AWS](/derrick/docs/deploy-aws)
- [GCP](/derrick/docs/deploy-google-cloud)
- [Getting Started with Nomad](/derrick/docs/get-started-nomad)
- [Getting Started with Kubernetes](/derrick/docs/get-started-kubernetes)
- [Getting Started with Docker](/derrick/docs/get-started-docker)
