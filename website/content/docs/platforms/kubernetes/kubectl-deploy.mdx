...
description: |-
  Derrick can mimic `kubectl apply` and deploy from a directory of YAML files. This option lets you adopt Derrick very easily with existing applications.
---

# Deploying Applications using YAML

Derrick can mimic `kubectl apply` and deploy from a directory of YAML
files. This option lets you adopt Derrick very easily with existing
applications. A downside of this option is that Derrick cannot help automatically
...
-> **There are other deployment options.** Derrick supports many other mechanisms
...
[Helm](/derrick/docs/platforms/kubernetes/helm-deploy).
...
We use the [`templatedir` function](/derrick/docs/derrick-hcl/functions/template#templatedir)
so that we can access the [`artifact` variable](/derrick/docs/derrick-hcl/variables/artifact)
...
functions in our YAML files is one big benefit of using Derrick.

The `prune_label` option specifies a Kubernetes label selector that Derrick
...
With this plugin, Derrick does not automatically inject the necessary
environment variables for the [Derrick Entrypoint](/derrick/docs/entrypoint). Therefore,
...
variables by using the [`entrypoint` variable](/derrick/docs/derrick-hcl/variables/entrypoint)
...
[manually inject the entrypoint](/derrick/docs/platforms/kubernetes/external-build#derrick-entrypoint-with-external-image-builds).
