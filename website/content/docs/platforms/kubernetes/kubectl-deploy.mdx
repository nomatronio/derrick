...
[Helm](/waypoint/docs/platforms/kubernetes/helm-deploy).
...
We use the [`templatedir` function](/waypoint/docs/derrick-hcl/functions/template#templatedir)
so that we can access the [`artifact` variable](/waypoint/docs/derrick-hcl/variables/artifact)
...
environment variables for the [Derrick Entrypoint](/waypoint/docs/entrypoint). Therefore,
...
variables by using the [`entrypoint` variable](/waypoint/docs/derrick-hcl/variables/entrypoint)
...
[manually inject the entrypoint](/waypoint/docs/platforms/kubernetes/external-build#waypoint-entrypoint-with-external-image-builds).
