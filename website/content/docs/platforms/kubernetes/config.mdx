...
description: |-
  Derrick provides multiple options for accessing ConfigMap and Secret resources within your deployed application. This can be used or instead of Derrick's native application configuration.
---

# ConfigMaps and Secrets

Derrick provides multiple options for accessing ConfigMap and Secret resources
within your deployed application. This can be used or instead of Derrick's native
[application configuration](/waypoint/docs/app-config) functionality.

This page will focus on Derrick-specific mechanisms for injecting
configuration into your application, and it all requires the
[Derrick Entrypoint](/waypoint/docs/entrypoint) to be injected and configured.
Please note that **if you're using the [Helm](/waypoint/docs/platforms/kubernetes/helm-deploy) or
[`kubernetes-apply`](/waypoint/docs/platforms/kubernetes/kubectl-deploy) deployment plugins,
you do not need to use the Derrick entrypoint** and you can access
...
in the `derrick.hcl` file using the [`config` stanza](/waypoint/docs/derrick-hcl/config).
Static environment variables like this can also be set using the
[web UI or CLI](/waypoint/docs/app-config).
...
in your Derrick configuration. A better idea would be to use a Kubernetes
Secret and access it from there. Derrick lets you do this with
[dynamic configuration](/waypoint/docs/app-config/dynamic):
...
This example shows two new concepts: [internal variables](/waypoint/docs/app-config/internal)
for creating intermediary values that can be used in other configuration
values and [files](/waypoint/docs/app-config/files) for creating configuration files.
...
Derrick will send a signal to the deployed application (by default `SIGUSR2`).
...
_after_ you had already deployed your application, Derrick would detect
...
For more information, including how to change the signal Derrick sends,
see [the reference documentation](/waypoint/docs/app-config/files#file-change-signal).
