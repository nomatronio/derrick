...
[application configuration](/derrick/docs/app-config) functionality.
...
[Derrick Entrypoint](/derrick/docs/entrypoint) to be injected and configured.
Please note that **if you're using the [Helm](/derrick/docs/platforms/kubernetes/helm-deploy) or
[`kubernetes-apply`](/derrick/docs/platforms/kubernetes/kubectl-deploy) deployment plugins,
...
in the `derrick.hcl` file using the [`config` stanza](/derrick/docs/derrick-hcl/config).
Static environment variables like this can also be set using the
[web UI or CLI](/derrick/docs/app-config).
...
[dynamic configuration](/derrick/docs/app-config/dynamic):
...
This example shows two new concepts: [internal variables](/derrick/docs/app-config/internal)
for creating intermediary values that can be used in other configuration
values and [files](/derrick/docs/app-config/files) for creating configuration files.
...
see [the reference documentation](/derrick/docs/app-config/files#file-change-signal).
