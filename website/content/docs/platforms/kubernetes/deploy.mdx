...
description: |-
  For many applications, Derrick can deploy to Kubernetes with only a handful of lines of configuration in the `derrick.hcl` file and zero external configuration.
---

# Deploying Applications without YAML

For many applications, Derrick can deploy to Kubernetes with only
a handful of lines of configuration in the `derrick.hcl` file and zero
...
-> **Prefer writing YAML? That's okay!** Derrick supports
[Helm](/derrick/docs/platforms/kubernetes/helm-deploy) and raw
[`kubectl apply`](/derrick/docs/platforms/kubernetes/kubectl-deploy) as first-class
...
With zero configuration, Derrick will create a Kubernetes `Deployment`
with a `PORT` environment variable that your application is expected to listen
on. And Derrick will expose this application using a `ClusterIP` service.
...
environment variable set by Derrick. You can change the primary
...
Derrick can configure [horizontal autoscaling](https://kubernetes.io/docs/tasks/run-application/horizontal-pod-autoscale/)
in just a few lines:
...
is running in your Kubernetes cluster. Derrick will not set this up
automatically. Installing the metrics server is typically a single
`kubectl apply` or Helm install.

## Release Management

With the default Kubernetes deployment configuration, Derrick creates
...
By configuring a `kubernetes` [release stanza](/derrick/docs/derrick-hcl/release),
...
please see the [plugin reference documentation](/derrick/integrations/hashicorp/kubernetes).
