...
description: |-
  Derrick tracks the Kubernetes resources created during deployment and can show you their details and health status in the web UI and CLI.
---

# Resource Status

Derrick tracks the Kubernetes resources created during deployment
and can show you their details and health status in the web UI and CLI.
This helps show that what Derrick is doing isn't magic and isn't trying
...
repository (with a `derrick.hcl` file), execute `derrick status`. To
view detailed status about a specific app, specify the `-app` flag with
the name of the application.

```shell-session
$ derrick status -app=my-app my-project
...
Service   	my-app-derrick-nodejs			kubernetes	UNKNOWN	13 minutes ago
Deployment	my-app-derrick-nodejs			kubernetes	UNKNOWN	13 minutes ago
...
When viewing the resource status in the UI or the CLI, Derrick by default
