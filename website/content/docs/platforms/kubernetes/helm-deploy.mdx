...
description: |-
  Derrick can deploy your applications onto Kubernetes using Helm. Derrick then tracks and shows you the health of all the resources that the Helm chart created.
---

# Deploying Applications with Helm

Derrick can deploy your applications onto Kubernetes using [Helm](https://helm.sh/).
Derrick then tracks and shows you the health of all the resources that
the Helm chart created.

If you're already using Helm, this lets you adopt Derrick with very little
friction since you can reuse your existing Helm charts for your applications.
Another benefit of using Helm with Derrick is that you can set chart values
more dynamically, such as setting different settings per environment,
referencing the most recently built artifacts, etc.

-> **Not a Helm user? No problem!** Derrick supports many other mechanisms
for deploying to Kubernetes. You can use the
[`kubernetes`](/derrick/docs/platforms/kubernetes/deploy) plugin for highly opinionated, no YAML deployments. Or you can use
[`kubernetes-apply`](/derrick/docs/platforms/kubernetes/kubectl-deploy) with
a directory of YAML files.

## Why Helm?

The Helm deployment plugin for Derrick is aimed primarily at people
...
[`kubernetes`](/derrick/integrations/hashicorp/helm/latest/components/platform) plugin. And you can always switch
...
you might still ask: but why would I use Helm with Derrick? Why not use
Helm directly? There are many reasons:

1. **Dynamic Chart values.** Derrick makes it easy to set chart values
   ...
   information using [HCL functions](/derrick/docs/derrick-hcl/functions/all).

2. **Environments.** Derrick handles the higher level multi-environment
...
4. **Other Derrick features.** You can take advantage of other Derrick features such as [remote operations](/derrick/docs/derrick-hcl/functions/all),
   [logs](/derrick/docs/logs), [exec](/derrick/docs/exec), [input variables](/derrick/docs/derrick-hcl/variables/input), and more.

## Using Helm with Derrick

The example below shows a `derrick.hcl` configuration that uses a
...
[here for more information on artifact variables](/derrick/docs/derrick-hcl/variables/artifact)). Notice that with the
`set` Chart values, we could use any Derrick variables, HCL functions, etc.
...
With this set, you can run `derrick deploy` (if you have an artifact
built already) and Derrick will use Helm to deploy your application.
...
[values YAML files](/derrick/integrations/hashicorp/helm/latest/components/platform).
...
If you want to use the [Derrick entrypoint](/derrick/docs/entrypoint) functionality,
you must manually inject the [entrypoint environment variables](/derrick/docs/entrypoint).
The [opinionated Kubernetes plugin](/derrick/docs/platforms/kubernetes/deploy) does this
...
A map of the environment variables to set are exposed in the `derrick.hcl`
as [`entrypoint.env`](/derrick/docs/derrick-hcl/variables/entrypoint). This can be
...
A full example is available [in the `derrick-examples` repository](https://github.com/nomatronio/derrick-examples/tree/main/kubernetes/nodejs-helm),
...
you must [inject the entrypoint manually](/derrick/docs/platforms/kubernetes/external-build#derrick-entrypoint-with-external-image-builds).
...
please see the [plugin reference documentation](/derrick/integrations/hashicorp/helm).
