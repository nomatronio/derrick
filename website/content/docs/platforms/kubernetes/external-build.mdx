...
[build](/waypoint/docs/lifecycle/build) step directly, but users may also use Derrick with
...
[input variables](/waypoint/docs/derrick-hcl/variables/input)
or [functions](/waypoint/docs/derrick-hcl/functions/all) to dynamically specify
the correct image.
...
[Input variables](/waypoint/docs/derrick-hcl/variables/input) can be used
...
This variable can then be used in [many ways](/waypoint/docs/derrick-hcl/variables/input#assigning-values-to-custom-input-variables).
...
workflow, would be to use [Git functions](/waypoint/docs/derrick-hcl/functions/all#gitrefhash).
...
The [Derrick Entrypoint](/waypoint/docs/entrypoint) is an optional component that enables
...
```dockerfile
FROM hashicorp/waypoint:latest AS derrick

# Your main build:
FROM yourbaseimage

COPY --from=derrick /usr/bin/waypoint-entrypoint /waypoint-entrypoint
