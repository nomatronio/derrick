...
description: |-
  Derrick incorporates a first-class build step that can be used to build your container images and other artifacts. This page will focus on container (or Docker) images, since these are the primary deployment artifact that Kubernetes uses.
---

# Building Container Images

Derrick incorporates a first-class [build](/derrick/docs/lifecycle/build) step that
...
build plumbed into my deployment configuration? Derrick solves both of these
by incorporating configuration for both build and deploys into the
[derrick.hcl file](/derrick/docs/derrick-hcl).

-> **The built-in build functionality is optional.** If you have an external
process already setup to build your images, you can
[use externally-built images](/derrick/docs/platforms/kubernetes/external-build). Derrick
...
prior to Derrick. However, Derrick supports many more methods of building
container images. Documentation on other methods along with examples can be
found on the [build lifecycle page](/derrick/docs/lifecycle/build).

## Docker Builder (Dockerfile)

The most popular way to build container images today is using a manually
written `Dockerfile`. Derrick can also build images from Dockerfiles
using the [`docker` builder](/derrick/integrations/hashicorp/docker/latest/components/builder). This also is
the easiest way to get started with Derrick with existing applications, since
most existing applications for Kubernetes have Dockerfiles.

-> **If you don't know Docker, or don't want to write a Dockerfile,**
take a look at the [`pack` builder](/derrick/docs/lifecycle/build#cloud-native-buildpacks).
...
This will use a `Dockerfile` in the same directory as your `derrick.hcl`
file. You can test this using the Derrick CLI by running `derrick build`. This
command will perform the build only, so it can be used for testing. The
`derrick build` command should be run in the same directory as your project
(with the `derrick.hcl`).

Derrick will automatically run your build remotely if you have a git repo
...
```shell-session
# let derrick choose to run your build remotely if possible (recommended):
$ derrick build
...
...
Because Derrick image builds happen remotely, they don't have access to your
local machine's secrets. The recommended approach to ensure secrets can be
accessed everywhere (locally and remote) is to use Derrick
[input variables](/derrick/docs/derrick-hcl/variables/input) paired with
...
the `derrick.hcl` and the `Dockerfile` used to build a Go application, as
an example.
...
[many different ways](/derrick/docs/derrick-hcl/variables/input#assigning-values-to-custom-input-variables). For long term use, the recommended location to set the variable
is via the web UI. However, for testing, you can use the CLI:

```shell-session
$ derrick build -var github_token=$MYTOKEN <project>/<app>
```

## Container Registry

After getting builds working, the next step is usually to push that container
image to a container registry. Derrick supports a
[`registry` stanza](/derrick/docs/derrick-hcl/registry) for specifying a
...
as we've done in the example above and run `derrick build` to verify it works.
This should push your image now.

### Hiding Credentials

In a real production use-case, you should not put your container registry
credentials directly in the `derrick.hcl` file. Instead, you should use
[input variables](/derrick/docs/derrick-hcl/variables/input) to parameterize them.
...
See [runner configuration](/derrick/docs/runner/config) for details.

## Local vs. Remote Builds

Derrick can run builds locally where you run the `derrick` CLI, or
remotely in your Kubernetes cluster in pods that are launched on demand.
By default, Derrick will automatically run your operations remotely when
[certain conditions](/derrick/docs/projects/remote#requirements) are met.
Builds triggered by Git polling or the UI also happen remotely.

To build Docker images within Kubernetes, Derrick uses
[Kaniko](https://github.com/GoogleContainerTools/kaniko). Derrick
