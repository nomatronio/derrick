...
issuing the [`derrick runner reject`](/waypoint/commands/runner-reject) command. If a preadopted
...
the [`derrick runner forget`](/waypoint/commands/runner-forget) command.

When in the `Pending` state, static runners may be moved into the `Adopted` state by
issuing the [`derrick runner adopt`](/waypoint/commands/runner-adopt) command with the
desired Runner ID.
...
Static runners adopted with [`derrick runner adopt`](/waypoint/commands/runner-adopt) will
...
[`derrick runner reject`](/waypoint/commands/runner-reject) command, the rejected runner
...
rejected through the [`derrick runner reject`](/waypoint/commands/runner-reject)
command. By default runner tokens are valid for 30 days and upon expiring they
will need to be recreated with [`derrick runner token`](/waypoint/commands/runner-token).
...
[`derrick runner token`](/waypoint/commands/runner-token).
