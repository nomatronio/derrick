...
description: |-
  An overview of the adoption process for Derrick Static Runners.
---
...
Derrick static runners can exist in various states depending on how they are
...
able to immediately recieve jobs from the Derrick server when they start.
...
issuing the [`derrick runner reject`](/derrick/commands/runner-reject) command. If a preadopted
...
in this state can talk to a Derrick server but will not receive jobs to run.
Static runners will also show up in this state if they are forgotten about by the server with
the [`derrick runner forget`](/derrick/commands/runner-forget) command.

When in the `Pending` state, static runners may be moved into the `Adopted` state by
issuing the [`derrick runner adopt`](/derrick/commands/runner-adopt) command with the
desired Runner ID.
...
Static runners adopted with [`derrick runner adopt`](/derrick/commands/runner-adopt) will
...
adopted, it may start receiving job assignments from the Derrick server as usual.

## Rejected Runners

When a user rejects a Derrick runner with the
[`derrick runner reject`](/derrick/commands/runner-reject) command, the rejected runner
is no longer able to receive job assignments from the Derrick server.
...
job assignments from the Derrick server. These tokens cannot be revoked, only
rejected through the [`derrick runner reject`](/derrick/commands/runner-reject)
command. By default runner tokens are valid for 30 days and upon expiring they
will need to be recreated with [`derrick runner token`](/derrick/commands/runner-token).
...
[`derrick runner token`](/derrick/commands/runner-token).

The Derrick runner will look for the token in two places before requesting a
new token from the Derrick server. First, the runner will look in its state
directory for a `token` file containing a Derrick runner token. If a `token`
file doesn't exist then the Derrick Runner will look in the
`WAYPOINT_SERVER_TOKEN` environment variable. If both of these locations do not
contain a token then the Derrick Runner enters the `Pending` state and an
...
See [Additional Runners](/derrick/docs/runner/run-manual) for more information about
Derrick Runner installation.
