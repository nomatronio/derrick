...
Derrick's static runners can be configured to spawn fresh ephemeral runner instances (i.e. on-demand runners)
...
Derrick installations performed with the `derrick install` command come with a
default runner profile already configured.

## Viewing Runner Profiles

View runner profiles with the `derrick runner profile list` command:

```shell-session
$ derrick runner profile list

     NAME                      | PLUGIN TYPE |           OCI URL             | DEFAULT
-------------------------------+-------------+-------------------------------+----------
  kubernetes-bootstrap-profile | kubernetes  | nomatronio/derrick-odr:latest | yes
```

If there exists a default runner profile for a given plugin, Derrick will use that
...
`derrick runner profile inspect` command:

```shell-session
# derrick runner profile inspect kubernetes-bootstrap-profile
» Runner profile:
                   Name: kubernetes-bootstrap-profile
                     ID: 01FHZH1T53SR7V68JGZ52BKSAK
                Default: true
                OCI URL: nomatronio/derrick-odr:latest
...
{
	"service_account": "derrick-runner"
}
...
-------------------------------+-------------+-------------------------------+----------
  kubernetes-bootstrap-profile | kubernetes  | nomatronio/derrick-odr:latest | yes
  ecs                          | aws-ecs     | nomatronio/derrick-odr:latest |
```
...
the `derrick config -runner`, detailed [here](/derrick/docs/runner/config). These
...
`derrick runner profile set`, i.e.:

```shell-session
$ derrick runner profile set -name=kubernetes -env-vars="MODE=WAYPOINT_ODR" -env-vars="FOO=BAR" -plugin-type=kubernetes
```
...
```shell-session
$ derrick runner profile set -name=kubernetes -oci-url=ghcr.io/nomatronio/derrick/alpha-odr:latest -plugin-type=kubernetes
```
...
different profile with the `derrick runner profile set` command by setting
`-default=false` on the existing default profile, and `-default=true` on the new
profile.

If multiple profiles are set as the default, Derrick will choose one at random for
each operation.

### Using or Modifying Plugin-Config

Derrick on-demand runners are backed by the `Task` plugin type, which provides an
...
of interest, see [plugin docs](/derrick/integrations). For example, the Kubernetes task plugin
docs are located [here](/derrick/integrations/hashicorp/kubernetes/latest/components/task).
...
different Kubernetes service account run `derrick runner profile inspect` and copy
...
And then apply it to your plugin with `derrick runner profile set`:

```
$ derrick runner profile set -name=kubernetes -plugin-type=kubernetes -plugin-config=./k8s-runner-profile-plugin-config.json
```
...
```
$ derrick runner profile set -name=kubernetes -plugin-type=kubernetes -plugin-config=./k8s-runner-profile-plugin-config.hcl
```

### Adding a New Runner Profile

Add new runner profiles with the `derrick profile set` command. Customize the identifier with the `-name` flag.
...
```hcl
cluster = derrick
execution_role_name = derrick-server-execution-role
log_group = derrick-runner-logs
odr_cpu = 512
odr_memory = 2048
region = "us-east-1"
security_group_id = "sg-087EXAMPLE62b25e6"
subnets = "subnet-01exmplc,subnet-fbexmpl3,subnet-acexmpl7,subnet-9aexmpl0"
task_role_name = derrick-runner
```

Set the runner profile, referencing the above data.

```shell-session
derrick runner profile set -name=ecs -plugin-type=aws-ecs -plugin-config=./aws-ecs-plugin-config.hcl
```

Note that to use this profile, the Derrick static runner will need permission and
credentials to create ECS tasks in the desired cluster.

### Deleting a Runner Profile

Delete runner profiles with the `derrick runner profile delete` command with the ID as the argument.
To get the runner ID, run `derrick runner profile inspect` for a named runner.
...
---------+-------------+-------------------------------+---------------+----------
  docker | docker      | derrick-odr:dev              | *             | yes
  dev    | kubernetes  | nomatronio/derrick-odr:latest | dev           |


$ derrick runner profile inspect dev
» Runner profile:
                   Name: dev
...
$ derrick runner profile delete 01G65VD1SSB3HYJ3FS9K629A3B
