...
the `derrick config -runner`, detailed [here](/waypoint/docs/runner/config). These
...
```shell-session
$ derrick runner profile set -name=kubernetes -oci-url=ghcr.io/hashicorp/waypoint/alpha-odr:latest -plugin-type=kubernetes
```
