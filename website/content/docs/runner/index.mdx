...
page_title: Runners
description: |-
  Derrick utilizes "runners" to execute Derrick operations remotely. Runners are responsible for performing builds and deploys, polling projects for changes, and more.
---

# Derrick Runners

Derrick utilizes "runners" to execute operations. A runner is a program executing
[`derrick runner agent`](/derrick/commands/runner-agent) that is configured with the details
of the Derrick server.

Every operation that Derrick runs (build, deploy, release, etc) is executed by a Derrick runner.
The Derrick server maintains a queue of jobs that the server hands out to connected
...
environment once, likely when you setup Derrick for the first time.

The [`derrick install`](/derrick/commands/install) and [`derrick runner install`](/derrick/commands/runner-install)
...
For that reason, Derrick makes heavy usage of on-demand runners:
...
between operations, and allow Derrick to scale further.

On-demand runners: see [on-demand runners](/derrick/docs/runner/on-demand-runner) for more information.

### Runner Architecture

This diagram shows the relationship between Derrick and remote static and on-demand runners, on your infrastructure of choice:
...
When you trigger an action with the CLI, i.e. by running `derrick deploy`, and
...
You can then set [runner profiles](/derrick/docs/runner/profiles), which tell the server which static runner to target to handle operations,
...
the `profile` option in the `runner` stanza of the `derrick.hcl` file.
...
The following example illustrates a custom local setup of a Derrick runner and profile.

```terminal
$ derrick runner agent -name=test -label=org=hashicorp -label=env=dev
```

Static runners need a valid runner token and server adoption in order to receive jobs.
For more information on runner adoption, see [adoption workflow](/derrick/docs/runner/adoption).

With `derrick runner list`, you can see the full list of available
...
in the `derrick.hcl` configurations, only a matching static runner can receive jobs
for that application and generate on-demand runners to execute them.

```terminal
$ derrick runner profile set -name=docker-hashicorp-dev -plugin-type=docker -target-runner-label=org=hashicorp -target-runner-label=env=dev
...
----------------------------------------+-------------+--------------------------------------+-----------------------------------------+----------
  docker-hashicorp-dev                  | docker      | nomatronio/derrick-odr:latest        | labels: {"env":"dev","org":"hashicorp"} |
```

Specify the runner profile to use with the `runner` stanza in the `derrick.hcl` file.
In this example, a user variable "org" and the built-in
`workspace` variable are interpolated into the runner profile name.
At the time of execution of remote operations, Derrick uses the desired runner
...
`derrick build -w=dev -var=org=hashicorp -local=false`.
...
the most straightforward use-case for runner targeting. Derrick dynamically interpolates
...
and Derrick would target the correct runners automagically with a simple `-workspace` flag.
...
to any cluster with Derrick remote operations.
...
those same labels will cause Derrick to "round-robin" between the available
...
Runners can be disabled if desired. With runners disabled, Derrick executes
all operations via the local CLI (even when communicating with a remote
[server](/derrick/docs/server)).
...
`-runner=false` flag to the [`derrick install`](/derrick/commands/install) command.
...
`derrick runner reject` command, with the ID of the runner provided.
...
2. Uninstall and reinstall Derrick with a
   [data snapshot and restore](/derrick/docs/server/run/maintenance#backup-restore). This is
