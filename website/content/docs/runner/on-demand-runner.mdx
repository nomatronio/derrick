...
description: |-
  A brief overview about on-demand runners and their role in Derrick.
---

# On-Demand Runners

On-demand runners allow a single Derrick installation to scale by
leveraging the defined platform to spawn resources to execute Derrick jobs.
Through on-demand runner profiles and static runner targeting, Derrick can
...
runner starts, it will execute the normal Derrick runner code and connect back
...
When Derrick goes to launch an on-demand runner task, it will generate four
jobs:
...
Users can configure Derrick server to launch on-demand runners with specific
configurations. These configurations are set through on-demand runner profiles.
See [runner profiles](/derrick/docs/runner/profiles) for more information.

## OCI/Docker Image

By default, the server will launch on-demand runners using the same OCI image the
server was configured with. For HCP Derrick, there is no default. The on-demand
configuration also accepts a specific OCI image URL to use instead. This allows
operators the ability to prepackage any Derrick plugins into the image and use
them in their configuration.

The official on-demand runner container image is built very similarly to the Derrick
server, with some additional packages such as `kaniko` for doing in-container
builds for jobs like the `Build` operation.

See the [runner profile docs on OCI URLs](/derrick/docs/runner/profiles#oci-url) for more information.

## Profile Configuration

To customize the environment and other launch configurations of an on-demand runner,
please consult the [on-demand runner profile documentation](/derrick/docs/runner/profiles).

Those docs detail how to configure and communicate the various aspects of an on-demand runner launch.

## Official Derrick Supported Task Plugin Platforms

We currently support the following platforms for launching on-demand runner tasks:

- [Kubernetes](/derrick/integrations/hashicorp/kubernetes/latest/components/task)
- [HashiCorp/Nomad](/derrick/integrations/hashicorp/nomad/latest/components/task)
- [AWS/ECS](/derrick/integrations/hashicorp/aws-ecs/latest/components/task)
- [Docker](/derrick/integrations/hashicorp/docker/latest/components/task)

-> **Self-managed Derrick server:** When you use the `derrick server install` command,
...
using the [plugin SDK](/derrick/docs/extending-derrick).
...
execute the Derrick runner code.

## Task Inspection

The CLI command `derrick task` gives users more insight into the task launcher system and
what Derrick does with remote-enabled projects. Tasks are the operations that
on-demand runners are built off of within Derrick.
...
» Derrick On-Demand Runner Tasks
...
You can also take a closer look at an individual task using `derrick task inspect`.
...
```shell
$ derrick task inspect 01GBWYSZ02YEB3EHMVQCA5240G
