...
Additional static runners can be added using [`derrick runner install`](/waypoint/commands/runner-install), which automates the process of creating
...
To install a static runner, use the [`derrick runner install`](/waypoint/commands/runner-install)
command. Below is an example of using that command to install a Derrick static runner to
Docker:

```shell-session
$ derrick runner install -platform=docker -docker-runner-network=derrick -server-addr=waypoint-server:9701 -server-tls-skip-verify=true
...
```shell-session
$ derrick runner install -platform=docker -server-addr=waypoint-server:9701 -docker-runner-network=derrick -server-tls-skip-verify
```

Kubernetes:

```shell-session
$ derrick runner install -platform=kubernetes -server-addr=waypoint-server:9701 -server-tls-skip-verify
```
...
```shell-session
$ derrick runner install -platform=nomad -server-addr=localhost:9701 -nomad-host-volume=waypoint-runner -server-tls-skip-verify
```
