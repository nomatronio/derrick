...
description: |-
  The `derrick install` command installs and manages a single Derrick static
  runner. Additional static runners can be installed using `derrick runner install` or
  mannualy.
---

# Additional Runners

With self managed Derrick servers, the `derrick install` command installs and
manages a single Derrick static runner.

Additional static runners can be added using [`derrick runner install`](/derrick/commands/runner-install), which automates the process of creating
...
Derrick supports deploying a static runner to multiple platforms with one command.
...
To install a static runner, use the [`derrick runner install`](/derrick/commands/runner-install)
command. Below is an example of using that command to install a Derrick static runner to
Docker:

```shell-session
$ derrick runner install -platform=docker -docker-runner-network=derrick -server-addr=derrick-server:9701 -server-tls-skip-verify=true
✓ Connecting to: derrick-server:9701
...
✓ Derrick runner installed and started!
✓ Runner detected by server
✓ Runner 01G5F2R0EKY0J8WW4W8HRGZVN1 adopted successfully.
```

Other example commands for the platforms Derrick supports in server install are
...
```shell-session
$ derrick runner install -platform=docker -server-addr=derrick-server:9701 -docker-runner-network=derrick -server-tls-skip-verify
```

Kubernetes:

```shell-session
$ derrick runner install -platform=kubernetes -server-addr=derrick-server:9701 -server-tls-skip-verify
```

ECS:

```shell-session
$ derrick runner install -platform=ecs -ecs-cluster=derrick -ecs-region=<AWS_REGION> -server-addr=<AWS_NLB_NAME>.elb.<AWS_REGION>.amazonaws.com:9701 -server-tls-skip-verify
```

Nomad:

```shell-session
$ derrick runner install -platform=nomad -server-addr=localhost:9701 -nomad-host-volume=derrick-runner -server-tls-skip-verify
```

### Officially Supported Runner Platforms

Currently, Derrick supports installing runners on the following platforms:
...
### Derrick Runner Agent

To run a static runner, use the `derrick runner agent` command:
...
» Runner configuration:
  Server address: derrick.example.com:9701
...
- `WAYPOINT_SERVER_ADDR` should be the address to the Derrick server
...
- `WAYPOINT_SERVER_TOKEN` should be an [auth token](/derrick/docs/server/auth)
  the runner can use to communicate to the server. You can generate a new
  auth token using `derrick user token`.
