...
description: |-
  Derrick utilizes "runners" to execute Derrick operations remotely. Runners can be configured to have environment variables and files exposed during certain project operations.
---

# Runner Configuration

Runners may require additional configuration such as cloud credentials,
Docker registry credentials, etc. Derrick enables runners to be configured
...
You can set environment variables using [`derrick config`](/derrick/commands/config-set)
with the `-runner` flag:

```shell-session
$ derrick config set -runner -scope=global AWS_ACCESS_KEY_ID=abcd AWS_SECRET_ACCESS_KEY=1234
```
...
~> **Security note:** HCP Derrick stores these values encrypted within its backend
using HashiCorp Vault. If that's unsatisfactory, you can set the environment variables
manually when [manually running the runner](/derrick/docs/runner/run-manual).

~> **Security note:** On a self hosted Derrick server, these configuration values are
stored as plaintext in the Derrick server data file. If you do not want to store any
secrets on the Derrick server, you must set the environment variables manually when
[manually running the runner](/derrick/docs/runner/run-manual).
...
Consider an example where you're using a `dev` and a `prod` workspace in Derrick.
...
your `dev` and `prod` environments are separated into different namespaces by the name names. These two different workspaces need to be configured by Derrick to deploy
...
is used for the `dev` Derrick workspace, and another token with permissions to
submit jobs to the `prod` namespace for the `prod` Derrick workspace. Here's what
that could look like:

```shell-session
$ derrick config set -runner -workspace=dev NOMAD_TOKEN=abc123

$ derrick config set -runner -workspace=prod NOMAD_TOKEN=def456
...
### Via the `derrick.hcl` File

Environment variables for runners can also be set via the `derrick.hcl`
file. This uses the same syntax as the [`config` stanza](/derrick/docs/derrick-hcl/config):
...
Using the `derrick.hcl` file, you can configure data to be stored in files
...
[application configuration files](/derrick/docs/app-config/files).
...
When using the `derrick.hcl` format, runners may make use of
[dynamic values](/derrick/docs/app-config/dynamic) to source configuration
from external sources. The `runner` stanza also has access to
[internal values](/derrick/docs/app-config/internal) for composing multiple
values together.
...
To modify the way that Derrick launches on-demand runners, see [runner profiles](/derrick/docs/runner/profiles).
