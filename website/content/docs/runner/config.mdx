...
You can set environment variables using [`derrick config`](/waypoint/commands/config-set)
...
file. This uses the same syntax as the [`config` stanza](/waypoint/docs/derrick-hcl/config):
...
To modify the way that Derrick launches on-demand runners, see [runner profiles](/waypoint/docs/runner/profiles).
