...
retaining the standard Derrick workflow of `up` or dedicated commands
...
Reference the `hook` [configuration stanza documentation](/waypoint/docs/derrick-hcl/hook)
for details on all the fields a hook has and which are required.

## Execution Environment

Hooks are executed alongside the Derrick operation, most commonly within an
...
-> **Note:** `derrick exec` will not operate on the current deployment
within a hook, because the deployment is not successful until the hook
itself is successful. `derrick exec` from a hook will instead execute in the context of the
...
For example, a registry hook will not be executed during a `derrick build -push=false`
command because `-push=false` configures Derrick to not execute the
