...
description: |-
  Labels are key/value pairs that can be specified by the user and attached to Derrick operations. Labels can be used to specify any information that may be useful for the user, but are not directly interpreted by Derrick itself.
---

# Labels

Labels are key/value pairs that can be specified by the user and
attached to Derrick operations. Labels can be used to specify any information
that may be useful for the user, but are not directly interpreted by
Derrick itself. For example, this can be used for organizational information
...
```shell-session
$ derrick build -label a=1 -label b=2
```
...
Labels starting with `derrick/` are reserved for Derrick rather than
user specification. It is an error to try to set any labels starting with
`derrick/` and Derrick will not allow it. We call these "system labels".
The full set of system labels is listed below:

- `derrick/workspace` - The current workspace of the operation.
...
using the [`labels`](/derrick/docs/derrick-hcl/variables/labels) variable
within the `derrick.hcl` file. The linked documentation shares various
...
app "web" {
  deploy {
    use "kubernetes" {
      annotations = { for k, v in labels: "derrick/${k}" => v }
    }
  }
}
```
...
Labels can be used in various places in Derrick for filtering by using
...
used is [application configuration](/derrick/docs/app-config/scope#label-scoping).
