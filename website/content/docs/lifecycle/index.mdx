...
description: |-
  Derrick has an opinionated view on the minimum application lifecycle: build, deploy, and release. These are the core elements that Derrick orchestrates during a `derrick up`.
---

# Application Lifecycle

Derrick has an opinionated view on the minimum application lifecycle:
build, deploy, and release. These are the core elements that Derrick
orchestrates during a `derrick up`.

These build, deploy, and release stages can be broken down to the individual
commands of `derrick build`, `derrick deploy`, and `derrick release`. This allows
...
- A **deploy** takes a previously built _artifact_ and stages it onto the target
  deployment platform and is available via [deployment URLs](/derrick/docs/url#per-deployment-urls)
...
Derrick maintains a history of all operations. You can view this history
...
`derrick artifact` and `derrick deployment`. The CLI has less listing
...
[The Build page](/derrick/docs/lifecycle/build) explains more including using Docker and Buildpacks.
...
[The Deploy page](/derrick/docs/lifecycle/deploy) explains more including using Docker, Kubernetes and Nomad.
...
[The Release page](/derrick/docs/lifecycle/release) explains more.

## Hooks

A **hook** is a customizable command that executes before or after the `build`,
`registry`, `deploy`, or `release` operations.
Hooks execute on the [Derrick Runner](/derrick/docs/resources/internals/execution),
...
[The Hooks page](/derrick/docs/lifecycle/hooks) explains more.
