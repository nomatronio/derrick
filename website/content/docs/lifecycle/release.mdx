...
[deployment](/derrick/docs/lifecycle/deploy)
and opens it
...
**The release stage is optional.** See [default behavior](/derrick/docs/lifecycle/release#default-behavior)
...
`release` configuration is explicitly available in the Derrick project
configuration file.

### No Releaser

If no default releaser exists for the deployment platform and no
release is configured in the Derrick project configuration, then each
...
During the release stage, Derrick will prune unreleased deployments.
By default, Derrick keeps just one previous deployment for
...
on `derrick up` and `derrick release`. The `-prune` flag can be used to
...
we will support setting defaults on the server side, in the `derrick.hcl`
file, and via the UI.
...
```shell-session
$ derrick up -prune-retain=0
...

or

$ derrick release -prune-retain=0
...
```
