...
description: |-
  Derrick build lifecycle documentation
---
...
A build is triggered during a `derrick up` or using the dedicated
`derrick build` command.
...
Derrick includes these builtin plugins to build a Docker image for your app.

- [Docker Build](/derrick/integrations/hashicorp/docker/latest/components/builder)
- [Docker Pull Build](/derrick/integrations/hashicorp/docker-pull/latest/components/builder)
- [Cloud Native Buildpacks](/derrick/integrations/hashicorp/pack)

You can also create a [plugin](/derrick/docs/plugins) to extend Derrick with your own builder.
...
it should work well the Derrick Docker plugin.

Reference the [Docker plugin](/derrick/integrations/hashicorp/docker) for all configuration options.
...
This plugin performs a `docker pull`. The plugin injects the Derrick Entrypoint by
...
See the [derrick](https://github.com/nomatronio/derrick-examples)
...
Several `derrick.hcl` adjustments may be required to enable alternative builders.
...
1. The `service_port` variable of the deploy plugin must match the buildpacks port.
   Paketo buildpacks commonly use `8080` and the default Derrick `service_port` is `3000`.

Paketo buildpacks example `derrick.hcl`:
...
GCP buildpacks example `derrick.hcl`:
...
Reference the [AWS ECS plugin](/derrick/integrations/hashicorp/aws-ecs) documentation for an AWS Elastic Container Registry example.
...
Reference the file in `derrick.hcl`:
...
Derrick can read [dynamic config variables](/derrick/docs/intro/use-cases/dynamic-config-vault-dynamic-secrets)
directly from Vault:
...
This means at runtime, when Derrick goes to push the built application artifact,
...
Derrick invokes Docker APIs, so if you have authentication configured with
`docker login` or similar, then Derrick does not need to authenticate directly
...
is replaced with your actual registry username. Therefore Derrick does not need
...
`derrick.hcl` registry configuration when relying on Docker's authentication:
...
documentation](/derrick/docs/lifecycle/deploy#private-registries) for details on how to
