...
based on the current [workspace](/derrick/docs/workspaces) or
[labels](/derrick/docs/lifecycle/labels). This can be used to change
configurations between environments such as development, staging,
production or by specific metadata like region.

This documentation page will use the [deploy](/derrick/docs/lifecycle/deploy)
...
We can use [workspaces](/derrick/docs/workspaces) as one way to model environments
...
[label selectors](/derrick/docs/lifecycle/labels#label-selector-syntax).
...
must be manually set. See [labels](/derrick/docs/lifecycle/labels).)
...
the [`workspace` variable](/derrick/docs/derrick-hcl/variables/workspace).
