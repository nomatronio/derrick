...
description: |-
  Derrick uses an entrypoint binary to enable features such as application configuration, logs, exec, and more.
---

# Derrick Entrypoint

Derrick uses an entrypoint binary to enable features such as application
configuration, logs, exec, and more. In most cases if building containers, Derrick
...
application. See [disabling the entrypoint](/derrick/docs/entrypoint/disable) for more information.
...
1. [Download](https://releases.hashicorp.com/derrick-entrypoint/) the appropriate
   `derrick-entrypoint` binary for the platform your application will run on. This is
   _not_ your desktop operating system but the operating system your app will be
   deployed _to_.

2. Put the single binary Derrick entrypoint anywhere in your deployment
...
3. Configure your application startup to run the entrypoint first rather
   ...
   `node app.js`, change that to `derrick-entrypoint /usr/bin/node app.js`
...
The Derrick entrypoint introduces some small amount of overhead to your
deployments.
...
Entrypoint logs are also streamed and can be viewed via `derrick logs`. If
`derrick logs` is not working, the entrypoint logs are also emitted to stderr.
...
via the [derrick config CLI](/derrick/docs/app-config#setting-configuration-via-the-cli).
...
The Derrick entrypoint is designed to be resilient to failure scenarios
that it introduces.

### Server Unavailable on Startup

If the Derrick server connection cannot be established on startup
within 5 seconds, Derrick will "fail open" by starting the child
process immediately. Derrick will continue to retry to connect to the
server in the background.

This behavior allows Derrick to start the child process no matter what.

If you're using [application configuration](/derrick/docs/app-config), the configuration
will not be available to the application in this failure case. This may
cause the application startup to fail.

### Server Connection Lost

If the Derrick server connection is lost at any point during runtime,
the child process is unaffected. Derrick will retry to establish a
...
The Derrick entrypoint _does not_ open any network listeners. It _does not_
...
The Derrick entrypoint _does_ maintain a long-running outbound connection
to the Derrick server as well as the Derrick URL service (if enabled).
Most importantly for security, the Derrick server can initiate an
...
The Derrick entrypoint connects to the Derrick server using a token
...
in any way, it poses a minimal risk to Derrick as a whole.

The Derrick entrypoint can be [completely disabled](/derrick/docs/entrypoint/disable)
if that is desired. This will disable all entrypoint-related functionality
including app config, logs, exec, etc. You may also
[disable specific features](/derrick/docs/entrypoint/disable#disabling-specific-features)
