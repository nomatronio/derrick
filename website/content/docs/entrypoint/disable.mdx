...
# Disable the Derrick Entrypoint
...
be used to prevent the Derrick from ever being used by a deployment. The
latter can be used to disable the entrypoint in deployments that already
have it installed.

If the entrypoint is disabled, all
[functionality that requires the entrypoint](/derrick/docs/entrypoint#functionality)
...
You can disable only the [`derrick exec`](/derrick/docs/exec) functionality by
...
Therefore, it cannot be set with [application configuration](/derrick/docs/app-config).
It must be set at build time or by using functionality of your deployment
plugin.

The example below shows how to set this using the [Kubernetes plugin](/derrick/integrations/hashicorp/kubernetes):
...
When exec is disabled, the entrypoint will not allow `derrick exec` to
...
plugins, `derrick exec` works without the Derrick entrypoint. For example,
