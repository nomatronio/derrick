---
layout: docs
page_title: Using Derrick to Deploy Microservices to a Service Mesh
description: |-
  Derrick eases the deployment of microservices to a Consul service mesh
---

# Using Derrick to Deploy Microservices to a Service Mesh

Derrick eases the deployment of microservices to a service mesh. Some
...
consider for deploying each of the microservices. This use case shows how Derrick
can help with that.
...
service mesh using Derrick. HashiCorp Consul is the service mesh
...
datacenter. Each of these is built and deployed using Derrick.
...
3. Nomad cluster, integrated with Consul
   ...
   - Additional Consul resources created (found in the use case repo under `nuka-cola/`)
     ...
     - Consul mesh config entry
     - Consul mesh gateway proxy default
4. An HCP Derrick server
   - Local context connected to HCP Derrick
   - An OSS Derrick server will also work, as long as the runners in Nomad and Kubernetes can connect to it.
5. A Docker image registry
   - Credentials to push to the registry

## Deploy the Derrick Runners to Nomad and Kubernetes

Prior to deploying the applications to each cluster, Derrick runners must be
...
$ derrick runner install \
  -platform=nomad \
  -id=mr-house \
  -nomad-host-volume=derrick-runner \
  -server-addr=api.hashicorp.cloud:443 \
  -nomad-dc=nuka-cola
  -- -label=consul-dc=nuka-cola
```

After the installation to of a runner to Nomad and Kubernetes is complete, `derrick runner list`
...
to filter the runners on which a Derrick job will be executed. To read more about
remote operations and how they work, check out [the documentation](/derrick/docs/projects/remote).
...
--------------------------+-------------+-------------------------------+-------------------------------------+----------
  kubernetes-mr-new-vegas | kubernetes  | nomatronio/derrick-odr:latest | labels:                             |
                          |             |                               | {"consul-dc":"sunset-sarsaparilla"} |
  nomad-mr-house          | nomad       | nomatronio/derrick-odr:latest | labels:                             |
                          |             |                               | {"consul-dc":"nuka-cola"}           |
```
...
The Derrick configuration for `counter-api` is below:
...
Derrick will use the Docker plugin to build the `counter-api` as per the
...
Nomad profile (the `runner` stanza below should be configured in the Derrick
...
Following this, running `derrick up` will build the app, push the app image
to a registry, and then deploy it to Nomad.

```shell
# derrick.wpvars contains various input vars set that are required in the
# Derrick file
$ derrick up -p countdash -a counter-api -var-file=derrick.wpvars
```
...
With `counter-api` deployed, `counter-ui` can be deployed next. Its Derrick
...
Derrick will use the Docker Pull plugin to pull the UI image from DockerHub,
and push it to a new registry after adding the [Derrick Entrypoint](/derrick/docs/entrypoint).
...
a label selector, to ensure that the pods with the `derrick-app` label matching
...
that Derrick launches a runner in the Kubernetes cluster when building and
...
and deployment may commence with the `derrick up` command below:

```shell
# derrick.wpvars contains various input vars set that are required in the
# Derrick file
$ derrick up -p countdash -a counter-ui -var-file=derrick.wpvars
```
...
## Service Mesh Connectivity between Derrick Apps
