...
description: |-
  Derrick enables you to source config dynamically from various external data sources using config sourcer plugins - this use case depicts doing just that with Vault dynamic database secrets.
---

# Dynamic Secrets for Derrick with Vault

When an app is deployed with Derrick, its deployment can also be configured using Derrick!
...
The [HashiCorp Vault plugin for Derrick](/derrick/integrations/hashicorp/vault)
is one of the [config sourcer plugins](/derrick/docs/app-config/dynamic)
that is built-in to the Derrick binary. This plugin can be used to source static
...
Prior to using Derrick to deploy Nomad jobs, Nomad users have relied on
...
but in instances where Derrick is used to deploy an app, it can be abstracted away
by using Derrick’s dynamic configuration integration with Vault, even when not
deploying to a Nomad cluster. This use case goes over how this can be done in Derrick.

An example application, located at [this path](https://github.com/nomatronio/derrick-examples/tree/d83e0831c2751163dc88503249c6559e88a6d912/learn/dynamic-application-configuration)
in the derrick-examples GitHub repository, will eventually need read-only access
...
```shell
$ tee derrick-policy.vault.hcl <<EOF
path "database/creds/readonly" {
  capabilities = [ "read" ]
}
EOF

$ vault policy write derrick derrick-policy.vault.hcl
Success! Uploaded policy: derrick-policy

$ vault token create -policy=derrick -ttl=24h
Key                  Value
---                  -----
token                <VAULT_TOKEN>
...
token_policies       ["default" "derrick"]
identity_policies    []
policies             ["default" "derrick"]
```

Now that Vault is configured to produce dynamic database credentials, Derrick
needs to be configured to connect to Vault. A configuration source for the Vault
plugin will be added like so:

```shell
$ derrick config source-set -type=vault -config=addr=http://127.0.0.1:8200 -config=token=<VAULT_TOKEN>
Configuration set for dynamic source "vault"!
```

To double-check that the token and address are what is expected, `derrick config source-get` can be run:

```shell
$ derrick config source-get -type=vault
  KEY  |           VALUE
--------+----------------------------
...
made ready by adding it to the derrick.hcl file, and committing it to the git
repository for the project.

```hcl
project = “derrick-dynamic-config-vault”
...
variable "image_name" {
  decription = "Name of the Docker image for the app built by Derrick. This will be used with the tag to push an image to a Docker repository."
  type = string
}

variable "image_tag" {
  decription = "Tag of the Docker image for the app built by Derrick. This will be used with the name to push an image to a Docker repository."
  type = string
}
...
In the configuration above, Derrick has been informed that it must get a secret
from Vault at the path `database/creds/my-role`. To learn more about how Derrick
does this, see the [Derrick Entrypoint docs](/derrick/docs/entrypoint).
...
can confirm that the secret was successfully templated after running `derrick up`,
by checking the environment variables using `derrick exec`, and checking the application
logs using `derrick logs`:
...
with an actual secret value. The Derrick Entrypoint, built-in to the application image,
...
cluster, the secrets necessary to run the application will be provided by Derrick.

## Further Reading

- [Derrick dynamic app configuration](/derrick/docs/app-config/dynamic)
- [Derrick dynamic variables](/derrick/docs/derrick-hcl/variables/dynamic)
- [Vault config sourcer plugin docs](/derrick/integrations/hashicorp/vault/latest/components/config-sourcer)
