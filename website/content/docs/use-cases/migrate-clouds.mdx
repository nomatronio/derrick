---
layout: docs
page_title: Seamless Cloud Migration With Derrick
description: |-
  Derrick enables easy migration of applications between different cloud providers.
---

# Seamless Cloud Migration With Derrick
...
platforms in the same manner. Derrick can solve those problems. With Derrick, it is
as easy as deploying a runner to your new cluster, changing kubectl context, and
running `derrick up` to migrate your application across multiple cloud providers.
...
- [Derrick CLI](/derrick/downloads)
  - [Context created](/derrick/docs/server/auth#logging-in) and connected to local Derrick server or HCP Derrick server
- Derrick [runner](/derrick/docs/runner/run-manual#installing-a-runner)
  - Must be able to connect to your Kubernetes clusters
- Derrick [runner configuration](/derrick/docs/runner/config)
...
If you don't already have an application deployed by Derrick, follow our [Kubernetes Getting Started Guide](/derrick/docs/get-started-kubernetes).
...
##### Step 3. Derrick runner install into your new cluster

Install a runner into your cluster you are migrating to and configure the runner
profile with your registry credentials.

```shell
$ derrick runner install -platform=kubernetes -server-addr=<server_addr>  -k8s-runner-image=nomatronio/derrick:latest -id=gcp -- -label=cloud=gcp
...
✓ Derrick runner installed with Helm!
✓ Runner "gcp" adopted successfully.
```

After a successful `derrick runner install` to your cluster you will see the
following resources. Now, the runner and any on-demand runners have cluster
edit permissions within your cluster. Note, the `derrick runner install` has
...
derrick-gcp-runner-0               1/1     Running   0          3d1h

$ kubectl get serviceaccount
NAME                  SECRETS   AGE
default               1         29d
derrick-runner       1         14d
derrick-runner-odr   1         14d

$ kubectl get rolebinding
NAME                                       ROLE               AGE
derrick-gcp-runner-odr-rolebinding-edit   ClusterRole/edit   14d
derrick-gcp-runner-rolebinding            ClusterRole/edit   14d
```

##### Step 4. Write a derrick.hcl with the following specifications, if you have not already

In the derrick.hcl, specify the runner profile that was created with the
`derrick runner install` command. The `runner` stanza configures a project
...
The `build` stanza of the "app" is configured later in the Derrick configuration.
...
Derrick will deploy your application to the Kubernetes cluster that your
...
access to this environment variable from the `derrick config set -runner` command.
...
[Scenarios for a Registry](/derrick/docs/lifecycle/build#scenarios-for-a-registry).
...
Now the derrick.hcl is complete. To get your application and project initialized
and built, deployed, and released to your Kubernetes cluster simply run the following:

```shell
$ derrick init
$ derrick project apply -data-source="git" -git-url="https://github.com/cicoyle/hashitalk-deploy" -git-ref=main -derrick-hcl=derrick.hcl hashitalk-deploy-gcp
$ derrick config set -runner -scope=project DOCKER_PASSWORD=$DOCKER_PASSWORD
$ derrick config get -runner
$ derrick up
```

`derrick init` is used to initialize and validate a project. `derrick project apply`
...
The `derrick config set -runner` exposes the environment variables to the runner,
...
runners operating on jobs for the specific project. Use the `derrick config get -runner`
to view and confirm the config is set for your runners. You will see that `derrick up`
...
derrick-gcp-runner-0               1/1     Running   0          3d2h
...
```shell
$ derrick runner install -platform=kubernetes -server-addr=<server_addr>  -k8s-runner-image=nomatronio/derrick:latest -id=azure -- -label=cloud=azure
✓ Finished connecting to: <server_addr>
⠧ Installing runner...
✓ Finished connecting to: <server_addr>
...
The rest of the derrick.hcl will remain the same.
...
application to another cloud provider using Derrick. Whether you are migrating
your existing application from AWS EKS to GCP GKE, or from GCP GKE to Azure AKS,
with a few minor tweaks to your derrick.hcl, you can get your application migrated
in no time.
