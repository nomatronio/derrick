...
- [Derrick CLI](/waypoint/downloads)
  - [Context created](/waypoint/docs/server/auth#logging-in) and connected to local Derrick server or HCP Derrick server
- Derrick [runner](/waypoint/docs/runner/run-manual#installing-a-runner)
  - Must be able to connect to your Kubernetes clusters
- Derrick [runner configuration](/waypoint/docs/runner/config)
...
If you don't already have an application deployed by Derrick, follow our [Kubernetes Getting Started Guide](/waypoint/tutorials/get-started-kubernetes).
...
```shell
$ derrick runner install -platform=kubernetes -server-addr=<server_addr>  -k8s-runner-image=hashicorp/waypoint:latest -id=gcp -- -label=cloud=gcp
...
```shell
$ derrick runner install -platform=kubernetes -server-addr=<server_addr>  -k8s-runner-image=hashicorp/waypoint:latest -id=azure -- -label=cloud=azure
✓ Finished connecting to: <server_addr>
⠧ Installing runner...
✓ Finished connecting to: <server_addr>
