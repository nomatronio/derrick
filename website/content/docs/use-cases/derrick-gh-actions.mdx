---
layout: docs
page_title: Using GitHub Actions to Unlock GitOps Workflows with Derrick
description: |-
  Set up your GitHub Derrick repo with Actions to automatically deploy your app
---

# Using GitHub Actions to Unlock GitOps Workflows with Derrick
...
This guide will lead you through setting up GitHub Actions to run a Derrick pipeline _on push_ to various Git branches, as well as configuring GH Actions to continuously deploy through a scheduled cron job.

## Prerequisites

This guide assumes that you have already set up some kind of infrastructure to run a [Derrick Runner](/derrick/docs/runners/hcp-runners-on-your-infra), HCP Derrick access, or optionally a self-managed [Derrick server](/derrick/docs/server/run). We recommend either Kubernetes or [HashiCorp Nomad](/nomad) for your infrastructure.

You will need an account on GitHub, as well as a Derrick project committed to a Github repo. This is the repository we will use to write a GitHub action. If you don’t have a project yet, there are plenty of example Derrick projects in the [examples repository](https://github.com/nomatronio/derrick-examples) to get started with.

## Setting Up Derrick

HashiCorp provides a hosted version of Derrick that runs the server on HCP. This guide supports using both a self-managed Derrick server as well as HCP Derrick. The main difference is what Derrick server address you configure in your GitHub secrets which we will discuss in the next section. Follow the docs for installing your own [self-managed Derrick server](/derrick/docs/server/run), this guide will not go over setting it up.

If you are using [HCP Derrick](https://derrick.dev), you’ll need to make sure you install a runner on your infrastructure so that Derrick can run operations. On the platform where you will run your runner on your own infrastructure, you’ll want to configure your CLI context to talk to HCP Derrick, and then install a runner into your managed infrastructure. Notice that the _derrick runner install_ commands will change depending on what platform you are installing to.
...
derrick runner install \
  -platform=kubernetes \
  -server-addr=api.hashicorp.cloud:443 \
  -k8s-runner-image=nomatronio/derrick:latest
```
...
If you already have a Derrick CLI context configured and runners installed, you can retrieve the server address and token values from your default CLI context. Found via `derrick context inspect`:

```
cat ~/.config/derrick/context/_default.hcl
...
## Configuring Your Derrick For GitHub Actions

To use certain secrets within a GitHub action, we’ll want to figure our GitHub project with a few secrets:

- `WAYPOINT_SERVER_ADDR`: Derrick Server Address
- `WAYPOINT_SERVER_TOKEN`: Derrick Server Authentication Token
...
Don’t forget to configure the Derrick Authentication Token as well as another Action secret.

Note: Because the Derrick server address and token are both parameterized, that means switching from a self-managed Derrick server to HCP Derrick is as simple as updating these values under your GitHub settings rather than having to reconfigure your GitHub Action script.
...
name: 'Runs a Derrick deployment on merge'
on:
...
jobs:
  deploy:
    ...
    steps:
      ...
      - run: |
          # Install the latest Derrick CLI
          wget -O- https://apt.releases.hashicorp.com/gpg | gpg --dearmor | sudo tee /usr/share/keyrings/hashicorp-archive-keyring.gpg
          echo "deb [signed-by=/usr/share/keyrings/hashicorp-archive-keyring.gpg] https://apt.releases.hashicorp.com $(lsb_release -cs) main" | sudo tee /etc/apt/sources.list.d/hashicorp.list
          sudo apt update && sudo apt install derrick

          # Create a CLI context to hook up to a Derrick server
          derrick context create \
            ...
            -set-default \
            hcp-derrick-gh-runner

      - run: |
          # Run the “marathon” pipeline!
          derrick version
          derrick init
          derrick pipeline run -workspace=${{ github.ref_name }} \
            -remote-source=ref=$GITHUB_SHA \
            marathon
```

This config will automatically install the latest version of Derrick from HashiCorp’s release repository. It uses the secrets we set up in GitHub via `${{ secret.SECRET_NAME }}`. Finally, for each run of a pipeline, we will run that operation in the specific workspace mapped to the git branch via `-workspace=${{ github.ref_name }}` and if ran on the non-default branch, we will grab that commit for `-remote-source=ref=$GITHUB_SHA`. You could also change this script to run `derrick up`.

## Derrick Deployments on Push
...
- Try [HCP Derrick](/derrick/docs)
- Derrick Tetris on GitHub: https://github.com/briancain/waypoint-tetris
