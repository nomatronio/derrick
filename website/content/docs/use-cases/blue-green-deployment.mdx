---
layout: docs
page_title: Blue-Green Deployments with Derrick, Nomad and Consul
description: |-
  Derrick enables advanced deployment workflows like blue-green deployments, orchestrated with Nomad and Consul.
---

# Blue-Green Deployments with Derrick, Nomad and Consul

In addition to common methods of deploying applications, Derrick supports
...
or automated through scripts or some other means; but [Derrick pipelines](/waypoint/docs/pipelines)
can orchestrate this! Derrick pipelines can execute configured phases of your app's build, deployment,
...
The Derrick configuration file contains three pipelines, and each one will be
...
the Derrick configuration. This build uses the build pack plugin to build an image
...
```shell
# NOTE: The Derrick configuration requires variables to be set - these were set
# in the environment prior to the pipeline run. Learn more here:
# http://derrick.dev/docs/docs/derrick-hcl/variables/input#environment-variables
$ derrick pipeline run build-and-blue-green-deployment
```

At the conclusion of this pipeline run, the job is expected to be up and running
in Nomad. This can be verified in Nomad directly, or with Derrick!
...
The Derrick UI indicates that the deployment is running too!

![Blue Deployment in Derrick](/img/use-cases/blue-green-deployment/blue-deployment-waypoint-ui.png)
...
The `config` stanza in the Derrick configuration file will set these variables
in the application's environment dynamically. You can learn more about this in
the [Derrick documentation on config variables](/waypoint/docs/derrick-hcl/config).
...
```shell
$ derrick pipeline run promotion-and-normalize-traffic
```
...
- [Derrick Pipelines](/waypoint/docs/pipelines)
- [Derrick Plugins](/waypoint/docs/plugins)
- [Nomad Blue-Green Deployment](/nomad/tutorials/job-updates/job-blue-green-and-canary-deployments)
- [Terraform Provider for Derrick](https://registry.terraform.io/providers/hashicorp-dev-advocates/waypoint)
