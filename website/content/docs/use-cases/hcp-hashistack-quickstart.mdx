---
layout: docs
page_title: A Complete Cloud Ecosystem built with Terraform, HCP Derrick, HCP Vault, HCP Consul, and Nomad
description: |-
  Your enterprise application is only a 'terraform apply' and 'derrick up' away from life on the cloud.
---

# A Complete Cloud Ecosystem built with Terraform, HCP Derrick, HCP Vault, HCP Consul, and Nomad

In this use case, we will spin up our entire infrastructure with `terraform apply`,
then launch the application to an EC2 container within our secured internal network with `derrick up`.
Welcome to the HashiCorp stack magic.

The ingredients for this special dish are [Derrick](https://www.derrick.dev/),
[Terraform](https://www.terraform.io/), [Consul](https://www.consul.io/), [Nomad](https://www.nomadproject.io/), and
[Vault](https://www.vaultproject.io/).
We chose to use HashiCorp managed versions of Derrick, Consul, and Vault for one convenient HCP UI and
...
- [Derrick CLI](/waypoint/downloads)
...
write configuration files for [Terraform](/terraform/language) and [Derrick](/waypoint/docs/derrick-hcl/syntax).
...
We will write application deployment configurations as code, which [HCP Derrick](https://cloud.hashicorp.com/products/waypoint)
...
The Terraform and Derrick configuration files and a sample application live in [this repository](https://github.com/xiaolin-ninja/2048_hcp_hashistack).
...
## Set up HCP Derrick Context

HCP Derrick is currently in Beta, and a Terraform module to provision Derrick is still under development.

Head into your HCP UI and [follow this tutorial](/waypoint/tutorials/runners/hcp-runners-on-your-infra#set-up-the-environment)
to set up your local environment.

Navigate to Derrick in the HCP dashboard:
![HCP UI](/img/use-cases/hcp-hashistack-quickstart/hcp.png)

Then click on "Install New Runner", which will generate terminal inputs to set up the Derrick context and install a Runner on your chosen platform.
In this case, we choose Nomad with a host volume named "wp_runner".
![Derrick Install New Runner](/img/use-cases/hcp-hashistack-quickstart/waypoint-runner-install.png)
...
![Derrick Install New Runner CLI](/img/use-cases/hcp-hashistack-quickstart/wp-runner-install.png)

Export the nomad address to your local environment and install the runner after installing the HCP Derrick context,
and you're all set to use HCP Derrick.

The `derrick.hcl` file defines application deployment configurations.
...
You can [read more about Derrick HCL](/waypoint/docs/derrick-hcl) fundamentals,
or explore [advanced deployment workflows using custom pipelines](/waypoint/docs/pipelines).

Now, run `derrick init`:
...
HCP Derrick does not provide a URL service.
You can also deploy the application by [installing self-hosted Derrick](/waypoint/commands/install)
and running `derrick up` with a different context.

## Easy to Monitor Dashboards

After our `terraform apply` and `derrick up`, you should have a full HCP HashiCorp stack set up automatically and application deployed.
...
You can see the Nomad server, client, and Derrick runner.
...
and Derrick expands the HashiCorp stack by enabling deployment automation with custom pipelines.
...
so you can explore this sweet HashiCorp stack synergy with self-managed Derrick, Consul, Nomad, and Vault as well!

- [HashiCorp Cloud Platform](https://cloud.hashicorp.com/)
- [Consul Service Mesh](/consul/docs/connect)
- [Deploy an Application to Nomad with Derrick](/waypoint/tutorials/get-started-nomad/get-started-nomad)
- [Set Up a Nomad Cluster on AWS](/nomad/tutorials/cluster-setup/cluster-setup-aws)
- [Blue-Green Deployments with Derrick, Nomad, and Consul](/waypoint/docs/use-cases/blue-green-deployment)
- [Consul Deployment Guide](/consul/tutorials/production-deploy/deployment-guide)
- [Advanced Deployment Pipelines with Derrick](/waypoint/docs/pipelines)
