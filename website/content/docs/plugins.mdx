...
how to write plugins, see the [Extending Derrick guide](/waypoint/docs/extending-derrick).
...
[derrick configuration](/waypoint/docs/derrick-hcl/use).
...
with the [`use` stanza](/waypoint/docs/derrick-hcl/use) in your Derrick configuration.
...
2. In `./.waypoint/plugins/` relative to the `derrick.hcl` file.
...
```
2020-10-17T11:32:12.743-0700 [DEBUG] derrick.runner: plugin search path: job_id=01EMVXARPMVCPYWE4FPECZY30B path=[, .waypoint/plugins, /Users/mitchellh/.config/waypoint/plugins]
```
...
You can also use the explicit [`plugin` stanza](/waypoint/docs/derrick-hcl/plugin)
to define plugins.
...
To write a plugin, see the [Extending Derrick guide](/waypoint/docs/extending-derrick).
