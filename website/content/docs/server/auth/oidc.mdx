...
description: |-
  Derrick can be configured to use OpenID Connect (OIDC) for authentication. This enables you to use existing identity providers such as Google, Okta, GitLab, and more.
---

# OpenID Connect (OIDC)

Derrick can be configured to use OpenID Connect (OIDC) for
...
[derrick auth-method set oidc](/derrick/commands/auth-method-set-oidc) CLI command.
...
documentation on [redirect URLs](/derrick/docs/server/auth/oidc#redirect-urls) while
configuring your OIDC provider.

Once you have the required information, use the CLI:

```shell-session
$ derrick auth-method set oidc \
...
You can use the other commands in the `derrick auth-method` subcommand group
...
- `http://127.0.0.1/oidc/callback` - This must be set for `derrick login`
...
`derrick login -v 127.0.0.1:9701`. The `-v` flag will log a WARN-level
entry with all the claims for your account.

Once you know what claims you want access to, you must specify them
in the `derrick auth-method set oidc` command, and create a rule. For
example, for GitLab:
...
to restrict users to your Derrick server. For full documentation on the
...
When logging in with OIDC, Derrick will automatically create a new user
account within Derrick if one does not exist. Derrick uses the following
...
installation can access your Derrick server.
