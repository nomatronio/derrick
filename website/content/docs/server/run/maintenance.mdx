...
description: |-
  A Derrick server needs to be maintained to ensure smooth operation and to protect against unexpected events such as server crashes. This page outlines various maintenance-related topics for smooth operation of a Derrick server.
---

# Server Maintenance

A Derrick server needs to be maintained to ensure smooth operation and
...
For `derrick install`-based servers, logs are always very verbose (`-vvv`).
...
The Derrick server stores data into a single `data.db` file.

### Path to Database

For `derrick install`-based servers:

- **Docker** - `/data/data.db`
- **ECS** - `/derrick-data/data.db`
...
The Derrick server supports both an online and offline backup and restore.

#### Online Backup and Restore

To backup the database, use the [`derrick server snapshot`](/derrick/commands/server-snapshot)
...
```shell-session
$ derrick server snapshot backup.snap
```
...
You can then restore this snapshot using [`derrick server restore`](/derrick/commands/server-restore).
...
```shell-session
$ derrick server restore backup.snap
```

If you are running the Derrick server in an environment that will automatically
restart it for you (such as Kubernetes or Nomad), then you may pass the
`-exit` flag to `derrick server restore`. This will cause the Derrick server
...
via `derrick server snapshot`.

In the server data directory (where the `data.db` file would normally be),
copy the snapshot data and name the file `derrick-restore.db`. For example,
if you normally run the Derrick server as `derrick server -db=/data/data.db`
then you should copy the snapshot data to `/data/derrick-restore.db`.

The real database doesn't need to exist; the restore process also works with
new, fresh servers. However, the snapshot data must still be named `derrick-restore.db`.
...
Online snapshots are safe to restore against any version of Derrick.
...
all future versions of Derrick. If there are any exceptions, it'll be noted
