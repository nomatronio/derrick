---
layout: docs
page_title: Derrick Server in Production
description: |-
...
# Derrick Server in Production

Derrick is still a very new project and we're collecting information for a more
detailed guide on how to run Derrick in production. We will fill in this page
...
Derrick requires two different ports:
...
Derrick Servers expect to have ample bandwidth available for all client
...
Derrick Servers are not highly latency sensitive; bandwidth is much
...
The Derrick server stores all application log buffers in memory. These
...
Derrick requires TLS for all inbound connections. TLS cannot be disabled.
By default, Derrick will generate a self-signed certificate on startup.

Servers that are [run manually](/derrick/docs/server/run#manually-running-the-server)
(_not_ installed using `derrick install`) can specify custom TLS certificates
using the `-tls-cert-file` and `-tls-key-file` flags on the `derrick server run`
command. These flags should point to a PEM-encoded certificate file and
private key, respectively.

### TLS Rotation

Derrick servers specified with the `-tls-cert-file` and `-tls-key-file`
