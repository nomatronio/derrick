...
[`derrick server run`](/derrick/commands/server-run) command. This is meant for more advanced
...
specified in the documentation for the [`derrick server run`](/derrick/commands/server-run)
...
[connecting to a server](/derrick/docs/server#connecting).

## Upgrading

For details on upgrading the server, please see the
[general Derrick upgrade documentation](/derrick/docs/upgrading).

## Easy Installation

We provide an opinionated install method for users who wish to get
off the ground quickly on certain server platforms with [derrick install](/derrick/docs/server/install).
...
[`derrick server run` command](/derrick/commands/server-run).
...
a [CLI context](/derrick/docs/server/auth#cli) automatically. You can now verify
