...
To pass values between components, you can ask Derrick to inject a previous component's Output Value. For example, the
next component after the `Builder` is the `Registry` component. The following code snippet shows that by defining a
parameter for the `Binary` type, Derrick automatically injects the reference to the `Binary` type that was output from
...
```go
option go_package = "github.com/nomatronio/derrick/golang/builder";
```
