...
description: |-
  The various component interfaces for a Derrick plugin
---

# Plugin Interfaces and Components

A Derrick plugin is a binary which implements one or more Derrick components, each of which are related
...
with the Derrick command that triggers them.

![Plugin Components](/img/extending-derrick/components.png)

# Implementing Components

To extend a particular part of the Derrick application lifecycle, you create a component which is a Go
...
in the Derrick SDK are not called directly; instead, they require that you return a function. The following example shows
...
There is no specific signature for the function you return from `BuildFunc`. The Derrick SDK automatically injects the
specified parameters. In the previous example, the signature defines three input parameters and two return parameters.
As a plugin author, you determine which parameters you want the Derrick SDK to inject for you. These are made up of
