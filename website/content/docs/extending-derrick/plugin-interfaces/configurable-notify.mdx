...
description: |-
  How to implement the ConfigurableNotify component for a Derrick plugin
---

# ConfigurableNotify

https://pkg.go.dev/github.com/nomatronio/derrick-plugin-sdk/component#ConfigurableNotify

`ConfigurableNotify` is an optional interface you can implement to receive a call back after the configuration
has been decoded by the Derrick SDK. It has a single input parameter which is the configuration reference you
return from the `Config` method. Returning an error from `ConfigSet` would stop execution of the Derrick operation.
