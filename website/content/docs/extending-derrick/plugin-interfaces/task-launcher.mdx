...
description: |-
  How to implement the TaskLauncher component for a Derrick plugin
---

The `TaskLauncher` component makes possible launching Derrick [On-Demand Runners](/derrick/docs/runner/on-demand-runner)
in a given platform, to perform [remote operations](/derrick/docs/projects/remote).

https://pkg.go.dev/github.com/nomatronio/derrick-plugin-sdk/component#TaskLauncher
...
a function to be called by Derrick. The function returned by `StartTaskFunc`
...
with the [`derrick job get-stream` command](/derrick/commands/job-get-stream).
