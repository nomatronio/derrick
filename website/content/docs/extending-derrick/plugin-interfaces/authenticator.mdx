...
description: |-
  How to implement the Authenticator component for a Derrick plugin
---

# Authenticator

https://pkg.go.dev/github.com/nomatronio/derrick-plugin-sdk/component#Authenticator

The Authenticator component is executed when the derrick init command is called, Authenticator is enabled
by implementing two interfaces, ValidateAuthFunc and AuthFunc. Typically you implement Authenticator along
with another Component, for example you have a Platform component which deploys a derrick application to
Google Cloud Run. You could implement the Authenticator component to check that the GCP credentials are valid.

![Authenticator](/img/extending-derrick/authenticator.png)
...
ValidateAuthFunc is called when you run derrick init, this is where you would implement logic which checks that
...
error is returned then Derrick calls the AuthFunc method. An example implementation of VaultAuthFunc can be found
...
func (p *Deploy) validateAuth(
  ...
  // returning an error from ValidateAuthFunc causes Derrick
  // to call AuthFunc
  return err
}
```
