...
page_title: 'Status'
description: |-
  How to implement the Status component for a Derrick plugin
---

# Status

https://pkg.go.dev/github.com/nomatronio/derrick-plugin-sdk/component#Status

The `Status` interface is responsible for reporting on the current health
for resources which have been created by the derrick `deploy`
...
https://pkg.go.dev/github.com/nomatronio/derrick-plugin-sdk/proto/gen#StatusReport
...
type StatusReport struct {
  ...
  Health StatusReport_Health `protobuf:"varint,2,opt,name=health,proto3,enum=nomatron.derrick.sdk.StatusReport_Health" json:"health,omitempty"`
...
When picking what kind of `Health` your deployment or release has, a Derrick
...
example in the `derrick-plugin-examples` repo with the `filepath` plugin. In
this case, the `filepath` plugin creates a file when Derrick runs a deployment.
...
// The StatusFunc here tells the Derrick plugin SDK which function in this
// plugin to invoke when Derrick runs through its Status check
