...
description: |-
  How to implement the ConfigSourcer component for a Derrick plugin
---

# ConfigSourcer

https://pkg.go.dev/github.com/nomatronio/derrick-plugin-sdk/component#ConfigSourcer

`ConfigSourcer` can be implemented to dynamically load configs from external
sources such as [Vault](/derrick/integrations/hashicorp/vault/latest/components/config-sourcer) and [Terraform Cloud](/derrick/integrations/hashicorp/terraform-cloud/latest/components/config-sourcer).
Derrick uses dynamic config sourcers to run alongside the application. The
...
To write your own config sourcer, the Derrick [entrypoint](/derrick/docs/entrypoint)
...
plugin in your own fork of Derrick and build it from your fork to have your
plugin included. You can also make a pull request to the main repository of
Derrick if you think your plugin makes sense.

We intend to provide a better way for the entrypoint to load custom plugins
in the future that does not require users building their own version of Derrick
