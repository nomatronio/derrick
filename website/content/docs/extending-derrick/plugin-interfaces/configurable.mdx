...
description: |-
  How to implement the Configurable component for a Derrick plugin
---

# Configurable

https://pkg.go.dev/github.com/nomatronio/derrick-plugin-sdk/component#Configurable

All plugins can receive the configuration from the `derrick.hcl` file by implementing the Configurable interface.
...
![Build Lifecycle](/img/extending-derrick/build.png)
...
to a Go struct which you would like Derrick to populate with the details from a configuration stanza.
...
You can then return a reference to an instance of this Struct from the Config function. The Derrick SDK will call the
...
https://github.com/nomatronio/derrick/blob/a3eb9e6872a80ed9f3e5a650fda13bd46f592c99/builtin/google/cloudrun/platform.go#L510
