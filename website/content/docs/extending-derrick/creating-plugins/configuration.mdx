...
A component in Derrick is a Struct that implements one or more Derrick interfaces. If you take a look at the file
...
will be created and the source code's location. Derrick allows you to define a custom configuration that can be passed
to your components. The following example shows the configuration for a Derrick application that uses your new plugin.
...
Configuration files are defined as `HCL` and parsed by the Derrick application. It converts the `HCL` configuration and
passes it to your plugin. So that Derrick knows how the configuration parameters map to your internal structures. You
...
When Derrick parses the configuration step for the application, it looks to see if your component has implemented the
`Configurable` interface and, if so, calls the `Config` method from which a reference to your config struct is returned.
Derrick uses the reference and attempts to serialize the application configuration to it.
...
To validate configuration, Derrick components can use the `ConfigurableNotify` interface. `ConfigurableNotify` defines
the method `ConfigSet` called after Derrick has read the `HCL` config file and serialized it to the struct you returned
from `Config`.
...
Derrick will present to the user.
...
[Next - Implementing the Builder interface](/waypoint/docs/extending-derrick/creating-plugins/build-interface)
