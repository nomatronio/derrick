...
build:
	...
	go build -o ./bin/derrick-plugin-${PLUGIN_NAME} ./main.go
...
install:
	...
	cp ./bin/derrick-plugin-${PLUGIN_NAME}* ${HOME}/.config/derrick/plugins/
```
...
Compile Plugin
go build -o ./bin/derrick-plugin-gobuilder ./main.go
```
...
```shell
ls ./bin
derrick-plugin-gobuilder
```

Let's now install the plugin so that it can be used by the Derrick CLI.

## Installing the plugin

You can install your plugin using the `make install` command. Derrick will automatically
load plugins from certain know locations, these are:

- Same directory as any `derrick.hcl`
- <derrick_app_folder>/.derrick/plugins/
- \$XDG_CONFIG_HOME/derrick/plugins/

The `make install` command will copy the plugin to the Derrick config in your
...
Installing Plugin
cp ./bin/derrick-plugin-gobuilder /home/nicj/.config/derrick/plugins/
```

Now the plugin has been installed, let's create an example application which uses your new plugin.

[Next - Creating an Example Application](/derrick/docs/extending-derrick/creating-plugins/example-application)
