...
[derrick trigger create](/waypoint/commands/trigger-create).
...
```
curl -k -G -d "token=`derrick user token`" https://waypoint.server.address.example.com/v1/trigger/123ABC456DEF
```
...
```
curl -k -G -d "variables={\"tag\":\"latest\"}" -d "token=`derrick user token`" https://waypoint.server.address.example.com/v1/trigger/123ABC456DEF
```
...
```
$ curl -k -G -d "stream=true" -d "token=`derrick user token`" https://waypoint.server.address.example.com/v1/trigger/01FTS1A6H3EH4H4W1XY89AWZ84
...
...
