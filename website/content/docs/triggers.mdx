...
page_title: Triggers
description: |-
  This page covers how to use Derrick Triggers and their various use-cases.
---

# About Derrick Triggers

Trigger URLs are pre-configured Derrick lifecycle operations for execution
...
a project, running a Derrick Up operation on all applications in a project,
and more. Trigger URLs make it easy to run Derrick operations inside CI without
requiring the Derrick CLI or any other Derrick setup in your scripts. You will
...
that tells Derrick to execute the pre-configured action. Trigger URLs
...
The Derrick CLI comes with a subcommand that lets you configure and update
Trigger URL configurations.

Check out our CLI docs for the `derrick trigger` command, such as
[derrick trigger create](/derrick/commands/trigger-create).
...
right project, application, and workspace. Then, specify which [lifecycle operation](/derrick/docs/lifecycle)
you want the trigger to execute when run. You can also include a `description` or
`tags` for extra information for the user.

```sh
$ derrick trigger create \
      -project=hcp-packer \
      -app=backend-svc \
...
Trigger URL: https://derrick.server.address.example.com/v1/trigger/123ABC456DEF
```
...
or typo, you can easily modify an existing trigger with `derrick trigger update`.

Once a trigger has been created, you can make an HTTP request against the given
trigger URL to start and queue a job on Derrick server.

## Modifying an existing Trigger URL Configuration

If you wish to see all of your configured triggers inside a Derrick server,
run the `derrick trigger list` command:

```sh
$ derrick trigger list
» Trigger URL Configs
...
```sh
$ derrick trigger inspect 01FS582B3X8VW0251C1XDQ56RR
...
```sh
$ derrick trigger update -id=01FS582B3X8VW0251C1XDQ56RR -workspace=production
...
For now, you can use this ID to make a request to Derrick server to execute the
trigger.
...
In an upcoming version of Derrick, we will provide an HTTP endpoint for easy
...
```
curl -k -G -d "token=`derrick user token`" https://derrick.server.address.example.com/v1/trigger/123ABC456DEF
```
...
```
curl -k -G -d "variables={\"tag\":\"latest\"}" -d "token=`derrick user token`" https://derrick.server.address.example.com/v1/trigger/123ABC456DEF
```

### Streaming Job Output over HTTP

When starting a trigger URL request, Derrick server supports streaming
...
that are not authenticated, Derrick server does not allow for job streaming.
...
```
$ curl -k -G -d "stream=true" -d "token=`derrick user token`" https://derrick.server.address.example.com/v1/trigger/01FTS1A6H3EH4H4W1XY89AWZ84
...
...
> GET /v1/trigger/123ABC456DEF?stream=true&token=tokenstring HTTP/1.1
> Host: derrick.server.address.example.com
...
* Connection #0 to host derrick.server.address.example.com left intact
```
