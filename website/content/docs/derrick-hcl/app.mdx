...
Multiple `app` stanzas can be specified if Derrick should deploy multiple
...
`derrick deploy -app frontend`.
The `-app` flag is a global target, and can be used with all Derrick
commands.
...
The application name is used by Derrick for organizational purposes.
...
application name will cause Derrick to treat it as a _new_ application.
...
- `build` <code>([build][build])</code> - Describes how to build this
  application during `derrick up` or `derrick build`.

- `deploy` <code>([deploy][deploy])</code> - Describes how to deploy
  this application during `derrick up` or `derrick deploy`.
...
- `release` <code>([release][release]: nil)</code> - Describes how to release
  this application during `derrick up` or `derrick release`. See
  [default behavior](/derrick/docs/lifecycle/release#default-behavior) for how
...
- `url` <code>([url][url]: nil)</code> - Describes the [URL service](/derrick/docs/url)
  behavior for this application. If this isn't specified, the default settings
  configured on the Derrick server will be used.

[build] /docs/derrick-hcl/build 'Build Stanza'
[deploy] /docs/derrick-hcl/deploy 'Deploy Stanza'
[release] /docs/derrick-hcl/release 'Release Stanza'
[runner] /docs/derrick-hcl/runner 'Runner Stanza'
[url] /docs/derrick-hcl/url 'url Stanza'
