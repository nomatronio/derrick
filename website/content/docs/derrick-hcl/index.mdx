...
written in [HCL](/derrick/docs/derrick-hcl/syntax) or [JSON](/derrick/docs/derrick-hcl/syntax/json),
allowing both human and machine friendly formats.

## `derrick.hcl` Location

Derrick expects one `derrick.hcl` per [project](/derrick/docs/projects).
...
and associated directly with a [project](/derrick/docs/projects).
...
`-derrick-hcl` flag to the [`derrick project apply`](/derrick/commands/project-apply)
command.
...
variables in the dedicated [variables section](/derrick/docs/derrick-hcl/variables).
...
- `project` `(string)` - The name of the project. This should be unique
  ...
  using [remote operations](/derrick/docs/projects/remote), including those
  triggered via [Git polling](/derrick/docs/projects/git).**
...
- `plugin` <code>([plugin][plugin])</code> - External plugins that may be
  ...
  [`use`](/derrick/docs/derrick-hcl/use) stanzas so this is only required if you
  need to additionally configure a plugin.

[app]: /derrick/docs/derrick-hcl/app 'App Stanza'
[plugin]: /derrick/docs/derrick-hcl/plugin 'Plugin Stanza'
