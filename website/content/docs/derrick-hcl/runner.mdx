...
The `runner` stanza configures a project for [remote operations](/derrick/docs/runner),
including configuring how to source a project's source code from Git and
configuring polling.

**The settings in this stanza only take effect when `derrick init -update`
is called.**

The `runner` stanza is completely optional. The settings specified in the
`runner `stanza can also be specified using the [`derrick project apply`](/derrick/commands/project-apply)
...
The above example enables [remote operations](/derrick/docs/runner) and configures
...
tying your `derrick.hcl` to a specific repository or putting secrets such
as Git auth information in your `derrick.hcl` file. See
...
CLI using [`derrick project apply`](/derrick/commands/project-apply) or via
the UI in the project create or modify screens. This approach is preferable
since it avoids having to put secrets in your derrick.hcl file and makes
your derrick.hcl more portable since it doesn't specify a repository.
...
- `enabled` `(boolean: false)` - This must be set to true to enable
  [remote operations](/derrick/docs/runner) from the CLI. This only affects
...
- `poll` <code>([poll][poll]: nil)</code> - Settings for polling the data
  source for changes and automatically running `derrick up`. By default
  polling is disabled.
...
- `url` `(string)` - The URL for the Git repository. This can be any URL supported
  by `git clone`. For example: `https://github.com/nomatronio/derrick-examples.git`
  or `git@github.com:nomatronio/derrick-examples.git`.
...
- `path` `(string: "")` - A relative path within the Git repository where
  ...
  `derrick.hcl` and sets the working directory when executing commands.
...
- `enabled` `(boolean: false)` - True to enable [polling](/derrick/docs/projects/git#polling).
  Polling requires that a valid datasource is configured.

- `interval` `(string: "30s")` - The interval between polling for changes.
  This defaults to 30 seconds.

[data_source]: /derrick/docs/derrick-hcl/config#data_source-parameters 'data_source Stanza'
[poll]: /derrick/docs/derrick-hcl/config#poll-parameters 'poll Stanza'
