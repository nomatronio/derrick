---
layout: docs
page_title: url - derrick.hcl
description: |-
  The `url` stanza configures the behavior of the [URL service](/derrick/docs/url) for the given application. This allows for certain applications to opt-out of the URL service, opt-in, etc.
---
...
The `url` stanza configures the behavior of the [URL service](/derrick/docs/url)
...
URL settings configured with the Derrick server will be used. The "default
settings" are the URL settings for the running Derrick server. These settings
...
- `auto_hostname` `(bool: <server configuration>)` - Whether to automatically
  ...
  If this is disabled, you can manually use the `derrick hostname` CLI to manage
