---
layout: docs
page_title: Expressions - Syntax - derrick.hcl
description: |-
  Expressions are used to refer to or compute values within a configuration. The simplest expressions are just literal values, like `"hello"` or `5`, but the Derrick configuration also allows more complex expressions such as arithmetic, conditional evaluation, and a number of built-in functions.
---
...
but the Derrick configuration also allows more complex expressions such as
references to attributes exported by previous stages, arithmetic, conditional evaluation,
and a number of built-in functions.

The rest of this page describes all of the expressions in Derrick's configuration language.
...
applied to it. Derrick configuration uses the following types for its values:
...
- `null`: a value that represents _absence_ or _omission._ If you set a
  parameter value to `null`, Derrick behaves as though you
...
Where possible, Derrick automatically converts values from one type to
another in order to produce the expected type. If this isn't possible, Derrick
will produce a type mismatch error and you must update the configuration with a
more suitable expression.

Derrick automatically converts number and bool values to strings when needed.
...
constant value. Derrick has a literal expression syntax for each of the value
...
- Strings are usually represented by a double-quoted sequence of Unicode
  ...
  Derrick, and have additional documentation on this page:
...
- Maps/objects are represented by a pair of curly braces containing a series of
  ...
  they are a valid [identifier](/waypoint/docs/derrick-hcl/syntax#identifiers), but must be quoted
...
Derrick configuration has a set of operators for both arithmetic and logic,
...
given values to be of a particular type. Derrick will attempt to convert
...
be of the _same_ type so that Derrick can determine what type the whole
conditional expression will return without knowing the condition value.

## Function Calls

Derrick configuration has a number of
[built-in functions](/waypoint/docs/derrick-hcl/functions) that can be used
...
[the function reference](/waypoint/docs/derrick-hcl/functions/all).
...
Derrick configuration has two different syntaxes for string literals. The
...
sequence. Derrick then processes the following lines until it finds one that
...
To improve on this, Derrick also accepts an _indented_ heredoc string variant
...
In this case, Derrick analyses the lines in the sequence to find the one
...
In both quoted and heredoc string expressions, Derrick supports template
