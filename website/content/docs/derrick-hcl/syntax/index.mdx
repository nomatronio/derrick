---
layout: docs
page_title: Syntax - derrick.hcl
description: |-
  This page describes the syntax of the `derrick.hcl` configuration language. The derrick.hcl file uses either HCL or JSON, allowing both human and machine friendly formats. The recommended and typical format is HCL.
---

# Configuration Syntax

This page describes the syntax of the `derrick.hcl` configuration language.
The derrick.hcl file uses either HCL or JSON, allowing both human and machine
...
order to use Derrick, and so this page summarizes the most important
...
The `derrick.hcl` configuration syntax is built around two key syntax constructs:
parameters and stanzas.
...
[expressions](/waypoint/docs/derrick-hcl/syntax/expressions) which allow the value to
...
All of the stanza types that Derrick supports are listed in the documentation
sidebar to the left. The documentation uses [placement tables](/waypoint/docs/derrick-hcl#placement-tables)
to document the context in which stanzas are valid.

## Comments

Derrick configuration supports three different syntaxes for comments:
...
For complete identifier rules, Derrick implements
[the Unicode identifier syntax](http://unicode.org/reports/tr31/), extended to
include the ASCII hyphen character `-`.

## Character Encoding and Line Endings

Derrick configuration files must always be UTF-8 encoded. While the
delimiters of the language are all ASCII characters, Derrick accepts
non-ASCII characters in identifiers, comments, and string values.

Derrick accepts configuration files with either Unix-style line endings
