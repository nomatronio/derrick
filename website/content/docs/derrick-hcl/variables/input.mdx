---
layout: docs
page_title: Input Variables - Variables - derrick.hcl
description: |-
  Input variables allow you to customize Derrick configuration according to
...
Input variables let you customize aspects of Derrick configuration with values
that may change, or that need to be set per Derrick run. Input variables can
also be used to [fetch values from remote sources](/derrick/docs/derrick-hcl/variables/dynamic).

You can parameterize your configuration by declaring variables in the
derrick.hcl file, and setting their values using CLI options, environment variables,
`auto` files, and the Derrick server UI.

Values used during a `build`, `deploy`, `release`, or `up` operation will output
to the CLI.

> **Hands-on:** Try the [Input Variables](/derrick/docs/configuration/input-variables)
> tutorial.

## Declaring an Input Variable

Each input variable referenced in the derrick.hcl file must be declared using a
[`variable` block](/derrick/docs/derrick-hcl/variable). Example variable declaration
blocks include:
...
be unique among all variables in the same derrick.hcl file. This name is used
to assign a value to the variable and to reference the variable's value when
evaluating the configuration.

The name of a variable can be any valid [identifier](/derrick/docs/derrick-hcl/syntax#identifiers).

### Arguments

Derrick CLI defines the following optional arguments for variable declarations:
...
default value will be used if no value is set when running Derrick. The
...
the command line. `derrick init` will fail to initialize plugins that rely
...
For example, given the below derrick.hcl file, you could run `derrick build`
without needing to pass in a value for the `port` variable. If you ran
`derrick up` or `derrick deploy` without specifying a value, you would
receive an error.
...
[type of value](/derrick/docs/derrick-hcl/syntax/expressions#types-and-values) that
...
allow Derrick to return a helpful error message if the wrong type is used.
...
Defining a variable as “sensitive” in the derrick.hcl definition looks like this:
...
Retrieve the server cookie using the command `derrick server cookie`:

```shell-session
$ derrick server cookie
WPC01FZ1FB3FQ1BYABXVN67FAPXJE
```

Then verify by combining the cookie with the value you expected Derrick to use:
...
Within the derrick.hcl file that declared a variable, its value can be accessed
from within [expressions](/derrick/docs/derrick-hcl/syntax/expressions) as
...
[Top-level parameters](/derrick/docs/derrick-hcl#top-level-parameters), such as the
...
number of ways. Derrick loads variables in the following order, with
later sources taking precedence over earlier ones:

- In the project settings via [the Derrick server's UI](/derrick/docs/server#derrick-server).
...
when running the `derrick up` or specific stage commands (e.g. `derrick build`).
You can use the `-var` option multiple times in a single command to set several
different variables.

```shell-session
$ derrick up -var="port=8080"
```

```shell-session
$ derrick up -var='networks=["internal","dev-test"]' -var="port=8080"
```

```shell-session
$ derrick up -var='network_map={"internal":8080,"dev-test":9701}'
```
...
When you pass a variable value to Derrick from the Windows Command Prompt,
use double quotes " around the argument:

```shell-session
$ derrick up -var "name=value"
```
...
```shell-session
$ derrick up -var-file="testing.wpvars"
```

A variable definitions file uses the same basic syntax as Derrick language
...
Derrick also automatically loads any `auto` variable definitions files - files
...
Derrick will search for a value in `WP_VAR_port` followed by `PORT`.
If neither are set, the default `8080` is used as specified in the stanza.

This can be useful when running Derrick in automation, or when running a
sequence of Derrick commands in succession with the same variables.
For example, at a `bash` prompt on a Unix system:

```shell-session
$ export WP_VAR_port=8080 && derrick up
...
```

On operating systems where environment variable names are case-sensitive,
Derrick matches the variable name exactly as given in configuration, and
...
as default values on the Derrick [runner](/derrick/docs/runner).
...
runners, then use `derrick config set -runner key=val` where you can specify a
`-scope` of `global`, `project`, or `app`. See more details in our
[config set documentation](/derrick/commands/config-set).
Alternatively, if you are targeting a specific runner profile, you can use the
`derrick runner profile set -env-var=key=val -name=runner-profile` command where
...
[runner profile set documentation](/derrick/commands/runner-profile-set).
...
`variable` definition block in the derrick.hcl file.
...
Derrick's usual syntax for
[literal expressions](/derrick/docs/derrick-hcl/syntax/expressions#literal-expressions)
...
variables. For convenience, Derrick defaults to interpreting `-var` and
environment variable values as literal strings, which need only shell quoting,
and no special quoting for Derrick. For example, in a Unix-style shell:
...
to require a complex value (list, set, map, object, or tuple), Derrick will
...
definition in the derrick.hcl file, you will get an error.

To avoid this error, either declare a variable block for the value, or remove
the variable value from your Derrick call.

### Variable Definition Precedence

The above mechanisms for setting variables can be used together in any
combination. If the same variable is assigned multiple values, Derrick uses
...
Derrick loads variables in the following order, with later sources taking
precedence over earlier ones:

- In the project settings via [the Derrick server's UI](/derrick/docs/server).
- Automatically loaded variable definition files, named with the pattern
  `*.auto.wpvars`.
