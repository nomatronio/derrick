---
layout: docs
page_title: config - derrick.hcl
description: |-
  The `config` stanza configures [application configuration](/derrick/docs/app-config). Application configuration sets environment variables for your application that is either statically set or synced with an external system.
---
...
The `config` stanza configures [application configuration](/derrick/docs/app-config).
...
Application configuration can be set via the `derrick.hcl` file using
the `config` stanza as well as via the [CLI](/derrick/commands/config-set).

-> **To learn about application configuration more broadly** please also read the
[application configuration](/derrick/docs/app-config) docs. The dedicated application
configuration section goes into more detail about how application configuration
works with Derrick. This page is specifically about the `config` stanza
in the `derrick.hcl` file and _does not_ cover the application configuration
system more broadly.
...
[workspace and label-scoping documentation](/derrick/docs/app-config/scope).

## Syncing

Changes to `config` in the `derrick.hcl` file take effect only during
the scenarios listed below:

1. `derrick up` - A full build, deploy, release will resync configuration.
1. `derrick deploy` - This is useful when you want to redeploy but don't
   want to rebuild the application.
1. `derrick config sync` - This is useful when you want to update the
...
- `file` `(map<string>string: {})` - Files to write at runtime for the
  ...
  [templating functions](/derrick/docs/derrick-hcl/functions/template). See
  the [configuration file documentation](/derrick/docs/app-config/files) for more information.
...
- `internal` `(map<string>ConfigValue: {})` - Internal variables use the
  ...
  [internal values documentation](/derrick/docs/app-config/internal) for more information.

- `workspace` <code>([config](#config-parameters))</code> - Define
  [workspace-scoped configuration](/derrick/docs/app-config/scope#workspace-scoping).
  The label for the stanza is the name of a workspace to exactly match.

- `label` <code>([config](#config-parameters))</code> - Define
  [label-scoped configuration](/derrick/docs/app-config/scope#label-scoping).
...
[default value for an input variable](/derrick/docs/derrick-hcl/variables/dynamic).
