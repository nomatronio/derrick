...
The plugin name must match the name of a [built-in plugin](/derrick/integrations?flags=builtin)
or an [external plugin](/derrick/docs/plugins). The plugin will be validated as part of
the `derrick init` process.
...
the available parameters [documented here](/derrick/integrations/hashicorp/docker/latest/components/builder#parameters).
