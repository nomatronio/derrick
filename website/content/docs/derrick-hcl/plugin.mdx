---
layout: docs
page_title: plugin - derrick.hcl
description: |-
  The `plugin` stanza configures a plugin that will be used by the Derrick configuration. This can set settings such as the checksum of the plugin, the kind of features the plugin supports, etc.
---
...
The `plugin` stanza configures a plugin that will be used by the Derrick
configuration. This can set settings such as the checksum of the plugin,
the kind of features the plugin supports, etc.

The `plugin` stanza is **optional.** When you use a [`use`](/derrick/docs/derrick-hcl/use)
stanza, it implicitly defines a plugin that Derrick will require. You do not
...
will be named on the local system. See [plugin loading](/derrick/docs/plugins) for more
information.
...
- `type` <code>([type](/derrick/docs/derrick-hcl/plugin#type-parameters): nil)</code> - The
