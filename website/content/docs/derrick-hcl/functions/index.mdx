---
layout: docs
page_title: Functions - derrick.hcl
description: |-
  The `derrick.hcl` file uses HCL which is able to call predefined functions. Functions enable Derrick configurations to represent complex configurations using programming techniques.
---

# Functions in `derrick.hcl`

The `derrick.hcl` file uses [HCL](https://github.com/hashicorp/hcl) which
is able to call predefined functions. Functions enable Derrick configurations
to represent complex configurations using programming techniques.

Most functions can be used anywhere in the Derrick configuration file.
Some functions are context-specific and are only valid when called from the correct locations.
Function documentation pages may use [placement tables](/waypoint/docs/derrick-hcl#placement-tables)
...
Derrick does not support custom defined functions.
...
For a complete list of variables, see [variables](/waypoint/docs/derrick-hcl/variables)
