...
The `hook` stanza configures [hooks](/derrick/docs/lifecycle/hooks) that are
...
[documented on the hooks lifecycle page](/derrick/docs/lifecycle/hooks#execution-order).
...
- `on_failure` `(string: "fail")` - Behavior when the hook fails. If this is
  ...
  operation to fail. See [failure behavior](/derrick/docs/lifecycle/hooks#failure-behavior).
