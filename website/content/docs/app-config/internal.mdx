...
Internal values are static or [dynamic](/derrick/docs/app-config/dynamic)
configuration values that can be referenced by environment variables,
[files](/derrick/docs/app-config/files), or other internal variables to compose
...
Internal values are declared in the `derrick.hcl` file in the
[`config` stanza](/derrick/docs/derrick-hcl/config). The example below configures
