...
description: |-
  Derrick can write application configuration to files. This is useful for applications that expect their configuration at a certain path or in a certain format.
---

# Configuration Files

Derrick can write application configuration to files. This is useful for
...
`derrick.hcl` file in the [`config` stanza](/derrick/docs/derrick-hcl/config).
...
[internal values](/derrick/docs/app-config/internal) which allow files to be built
up of dynamic values as well. When a value
changes at runtime, Derrick will re-render your configuration file and
send a signal to your application.

## File Change Signal

If the input variables to a configuration file change at runtime, Derrick
will rerender the file and signal your application. By default, Derrick will
send the signal `SIGUSR2`. Derrick expects your application to respond to this
signal by reloading the configuration.

The file change signal can be customized using the
[`file_change_signal` field](/derrick/docs/derrick-hcl/config#file_change_signal)
