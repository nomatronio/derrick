...
Deleting the key from the Derrick file and [syncing](/waypoint/docs/app-config#configuration-syncing)
...
To set configuration source settings, the [`derrick config source-set`](/waypoint/commands/config-source-set) CLI command is used:
...
```shell-session
$ derrick config source-set -scope=global -type=vault -config=addr=https://vault.service.consul:8200 -config=token=waypoint-global-token
...
is [`derrick logs`](/waypoint/commands/logs). The logs command will show entrypoint logs where
you will see any errors related to configuration syncing.

The [`derrick logs`](/waypoint/commands/logs) output below is from an application deployed with
...
set, open a shell with [`derrick exec`](/waypoint/commands/exec) and run `env`. This command will
