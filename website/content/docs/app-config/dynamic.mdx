...
description: |-
  Derrick can sync application configuration values with external systems such as Kubernetes ConfigMaps, HashiCorp Vault, Amazon Parameter Store, and more. As the values change, Derrick will automatically update your deployments and restart your application.
---
...
Derrick will automatically update your deployments and restart your application.
...
This functionality requires the [Derrick entrypoint](/derrick/docs/entrypoint).

-> **Basic application configuration knowledge required.** This
page assumes you know the basics of how to
[set static application configuration](/derrick/docs/app-config). This page will
not thoroughly explain the config setting CLI or `derrick.hcl` stanzas.
...
- [Terraform Cloud](/derrick/integrations/hashicorp/terraform-cloud/latest/components/config-sourcer) -
  Read Terraform state outputs from Terraform Cloud.
- [Kubernetes](/derrick/integrations/hashicorp/kubernetes/latest/components/config-sourcer) - Read values from
  ConfigMaps and Secrets.
- [Vault](/derrick/integrations/hashicorp/vault/latest/components/config-sourcer) - Read values from Vault secrets,
  including dynamic secrets such as database credentials.
- [AWS SSM](/derrick/integrations/hashicorp/aws-ssm/latest/components/config-sourcer) - Read configuration values
  from AWS SSM Parameter Store.
- [Consul](/derrick/integrations/hashicorp/consul/latest/components/config-sourcer) - Read values from Consul key
  value storage.
...
## Setting Dynamic Values via `derrick.hcl`

Dynamic configuration values are set in the `derrick.hcl` file using
...
changes, and [restarts your application](/derrick/docs/app-config#application-restart-behavior).
...
To unset a dynamic configuration value, delete it from the `derrick.hcl`
file and then use the CLI to set it to an empty string as shown below.
Deleting the key from the Derrick file and [syncing](/derrick/docs/app-config#configuration-syncing)
it will not be enough.

```shell-session
$ derrick config set PORT=
```
...
For example, [Vault](/derrick/integrations/hashicorp/vault/latest/components/config-sourcer) has some global
...
To set configuration source settings, the [`derrick config source-set`](/derrick/commands/config-source-set) CLI command is used:

```shell-session
$ derrick config source-set -type=vault -config="auth=aws"
```
...
The `derrick config source-set` command's `-scope` flag will configure a given
...
For example, you may have three config sources configured in your Derrick server,
...
```shell-session
$ derrick config source-set -scope=global -type=vault -config=addr=https://vault.service.consul:8200 -config=token=derrick-global-token

$ derrick config source-set -scope=project -p=foo -type=vault -config=addr=https://vault.service.consul:8200 -config=token=foo-project-token

$ derrick config source-set -scope=app -p=foo -a=bar -type=vault -config=addr=https://vault.service.consul:8200 -config=token=bar-app-token
```

In accordance with the scopes set, `derrick config source-get` will return the
config source corresponding with the specified `scope`.

```shell-session
$ derrick config source-get -scope=global -type=vault
» Config Source Info:
   Type: vault
  Scope: global
...
--------+----------------------------
  ...
  token | derrick-global-token

$ derrick config source-get -scope=project -p=foo -type=vault
Config Source Info:
       Type: vault
      Scope: project
    Project: foo
...
$ derrick config source-get -scope=app -p=foo -a=bar -type=vault
Config Source Info:
       Type: vault
      Scope: app
    Project: foo
...
```shell-session
$ derrick config source-get -scope=all
...
The Derrick Entrypoint will always attempt to retrieve configuration for the
...
```shell-session
$ derrick config source-set -scope=app -p=foo -a=bar -w=foobar -type=vault -config=addr=https://vault.service.consul:8200 -config=token=bar-app-foobar-workspace-token

$ derrick config source-get -scope=app -p=foo -a=bar -w=foobar -type=vault
Config Source Info:
       Type: vault
      Scope: app
    Project: foo
...
Derrick server is used for releasing applications to multiple environments. A
...
To unset the configuration after it has been set with `derrick config source-set`, specify the `-delete` flag.

```shell-session
$ derrick config source-set -type=vault -delete
```
...
is [`derrick logs`](/derrick/commands/logs). The logs command will show entrypoint logs where
you will see any errors related to configuration syncing.

The [`derrick logs`](/derrick/commands/logs) output below is from an application deployed with
Derrick using Kubernetes ConfigMaps but deployed within a Nomad cluster. The
...
To iterate on configuration, change the `derrick.hcl` file and run
`derrick config sync`. This allows you to change configuration without
...
set, open a shell with [`derrick exec`](/derrick/commands/exec) and run `env`. This command will
