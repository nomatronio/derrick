...
description: |-
  Derrick allows you to set different configuration values for specific workspaces or labels. For example, you might set a database URL for staging different from production using workspaces, or you might use labels to set different addresses for different regions or any other metadata.
---

# Workspace and Label Scoping

Derrick allows you to set different configuration values for specific
[workspaces](/derrick/docs/workspaces) or [labels](/derrick/docs/derrick-hcl/variables/labels).
...
By default, Derrick application configuration is exposed to the application
...
exact [workspaces](/derrick/docs/workspaces):
...
cannot set an [internal variable](/derrick/docs/app-config/internal) at the root
...
automatically are exposed via the `derrick/workspace` label and label
selectors support
...
config {
  ...
  label "derrick/workspace contains \"dev\"" {
    env = {
      "DB_ADDR" = "prod.example.com"
    }
  }
...
label selector of `derrick/workspace == <name>`.
...
configuration values, such as [internal values](/derrick/docs/app-config/internal).
