...
description: |-
  Derrick is designed to be flexible and resilient when upgrading from one version to the next. We've carefully thought out an upgrade process so you can predict what will be involved when upgrading Derrick.
---

# Upgrading Derrick

Derrick is designed to be flexible and resilient when upgrading from
one version to the next. We've carefully thought out an upgrade process
so you can predict what will be involved when upgrading Derrick.

Derrick is composed of many components which each have their own upgrade
considerations:

- **Client** (such as the CLI or UI) - Users of Derrick are not expected
  to exactly match the Derrick server version. The Derrick upgrade process
  and compatibility promise allows for clients with differing versions from
  a Derrick server.

- **Entrypoint** - The [entrypoint](/derrick/docs/entrypoint) is built and run
  directly on the deployment artifact. Users of Derrick cannot be expected
  to rebuild and redeploy all applications. The Derrick upgrade process
  and compatibility promise is designed so that entrypoints retain compatibility
  as long as possible.

- **Server** - The server component of Derrick must remain compatible
  with all actively used clients and entrypoints. The Derrick upgrade process
  is designed for you to know when and how to safely upgrade the server.

Please see our [compatibility promise](/derrick/docs/upgrading/compatibility)
...
If you're using the [Derrick Helm chart](https://github.com/hashicorp/waypoint-helm)
to install Derrick, you can upgrade using the `helm upgrade` command.
Please take note of any
[version-specific upgrade notes](/derrick/docs/upgrading/version-guides) prior
...
```shell-session
$ helm repo update
$ helm upgrade derrick nomatronio/derrick
```

-> Note that the "derrick" value in `helm upgrade` refers to the
release name you chose during `helm install`. This may be different for you.

### Server Upgrade Command

Derrick provides a command for users to upgrade the
Derrick server in place. For each platform, Derrick attempts to
...
When the upgrade command is executed, Derrick does the following:
...
- Runs the upgrade for the requested platform:
  ...
  - `kubernetes` - This platform will submit a patch request to kubernetes to
    set the image requested on the derrick-server pod. By default, Derrick's
    StatefulSet is configured to `RollingUpdate`, so the pod will pull the
    requested version shortly after running the Derrick upgrade command.
  - `nomad` - This platform will update the derrick server job with the server
    image requested.
  - `ecs` - This platform will locate the existing cluster, update task definitions, and upgrade the image to the latest.
- Verifies that derrick CLI can still connected and speak to Derrick server
...
A standard upgrade is one where the [protocol version](/derrick/docs/upgrading/compatibility)
...
2. [Take a snapshot of the server](/derrick/commands/server-snapshot)

3. Shut down the previous server version A.

4. Start the new server version B.

5. [Restore the server snapshot](/derrick/commands/server-restore)

6. [Verify the connection](/derrick/commands/context-verify)
...
2. Check the help text from running `derrick server upgrade -help`.

3. Ensure you are in the right server context before upgrading.
   Check current context with `derrick context inspect`. If this is not the intended context to upgrade,
   run `derrick context list` and choose the right one with `derrick context use <name>`.
   The upgrade automatically runs `derrick context verify`.

4. When ready, run the upgrade command for your given platform. For example:

```shell-session
derrick server upgrade -platform=kubernetes -auto-approve
```

This will automatically save a snapshot of the server in your current directory
prior to upgrading.

If the server was installed with a static runner, Derrick will uninstall the existing runner and install a new one with the new version.
...
[protocol version](/derrick/docs/upgrading/compatibility)
...
confidently upgrade your Derrick installations. Some of our roadmap plans
include:
