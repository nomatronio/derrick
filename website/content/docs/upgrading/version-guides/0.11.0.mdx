...
Derrick 0.10.x can be upgraded to Derrick 0.11.0 using a [standard upgrade](/derrick/docs/upgrading#standard-upgrade).
...
path must be set to `/derrickserverdata`. The POSIX user and root directory
...
spec:
  ...
  source:
    persistentVolumeClaimName: data-default-derrick-server-0
EOF

$ kubectl get volumesnapshot
NAME                READYTOUSE   SOURCEPVC                        SOURCESNAPSHOTCONTENT   RESTORESIZE   SNAPSHOTCLASS   SNAPSHOTCONTENT   CREATIONTIME   AGE
derrick-snapshot                data-default-derrick-server-0                                         <storage-class>                                              13s
```
...
<volume-id>@derrick-0-11-upgrade  <volume-id>  211MiB  2023-02-14-15:45-08:00  true

$ nomad job inspect derrick-server > derrick-server-0.10.json
$ # Make a copy of `derrick-server.json` and set {"Job": "Stop": false} and the derrick server image version to "0.11.0" in the "derrick-server" task group.
