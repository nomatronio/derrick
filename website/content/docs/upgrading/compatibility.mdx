...
description: Derrick version compatibility is determined based on the protocol version and not the release version such as "1.2.3". Two different Derrick release versions are compatible so long as their protocol versions are compatible.
---

# Compatibility Promise

Derrick has multiple components that need to interact with each other:
the clients, the server, entrypoints, etc. Due to the diverse nature of these
components, Derrick has a protocol versioning system designed to minimize
breaking changes and allow users to predictably run differing versions of
each component.

-> **Derrick version compatibility is determined based on the
[protocol version](#protocol-versions) and not the release version such
as "1.2.3".** Two different Derrick release versions are compatible so
long as their protocol versions are compatible. This allows for multiple
simultaneous Derrick versions to be in use, which is expected.

Our promises:

1. We promise that two Derrick versions can interoperate with each other if their
   [protocol versions](#protocol-versions) overlap.

2. We promise that there will always be a release with overlapping protocol
   versions to allow all Derrick components to safely upgrade.

3. For Derrick servers, we promise that each subsequent release will be compatible with the
...
separate Derrick processes to interoperate by API. Compatibility does
not cover CLI commands or flags, the Derrick configuration file, or UI
layout.
...
Derrick advertises _protocol versions_ which can be used to determine
...
For each advertised protocol version, Derrick advertises a _minimum_
compatible version along with the _current_ compatible version. A client
may communicate with a Derrick server so long as the current protocol
versions are within the range advertised.

Finally, Derrick advertises two types of protocol versions: one for
the API, and one exclusively for the [entrypoint](/derrick/docs/entrypoint) protocol.
This allows Derrick to iterate on the API in potentially backwards
incompatible ways without breaking entrypoints, and vice versa.

### Current Protocol Version Table

The current protocol version table can be viewed [here](/derrick/docs/upgrading/protocol-table).

### Example

The following is a concrete example of using the protocol versions to
determine compatibility. The Derrick versions and their advertised protocol
versions are examples and not real, current Derrick versions.

The scenario of the _currently deployed_ Derrick components is:
...
In this scenario, there is a diverse set of Derrick components currently
...
In this scenario, a new version of Derrick comes out with the following compatibility:
...
[upgrade guide](/derrick/docs/upgrading).
...
At the time of writing, Derrick is a relatively new project. Based on
...
With Derrick, we hope to achieve something similar: we may iterate on the
