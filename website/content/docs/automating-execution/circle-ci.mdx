---
layout: docs
page_title: Integrating Derrick with CircleCI
description: |-
  How to utilize Derrick with CircleCI as the service to run
  Derrick to build and deploy applications
---

# Integrating Derrick with CircleCI
...
configure a standalone Derrick setup temporarily, with the server and runner
...
2. **Install and Initalize Derrick with `derrick/init`.** This command will
   install the derrick CLI, start up a remote docker instance and run
   `derrick init`.
...
~> **Note:** The CircleCI `circleci/waypoint` orb is a legacy upstream integration.
> For new Derrick pipelines, install the `derrick` CLI directly or use a custom orb.
...
orbs:
  derrick: circleci/waypoint@1.0.0

jobs:
  build:
    executor: derrick/default
    environment:
      DERRICK_SERVER_ADDR: ''
