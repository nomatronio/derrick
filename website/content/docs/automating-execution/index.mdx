...
Derrick [Git integration](/waypoint/docs/projects/git). We recommend trying that
...
binary will need access to the [Derrick server](/waypoint/docs/server). Locally, it is recommended
...
- `WAYPOINT_SERVER_TOKEN`. Must be set to a Derrick token, created with [`derrick user token`](/waypoint/commands/user-token)
...
The [`derrick init` command](/waypoint/commands/init) is still required in remote environments, and must be
