---
layout: docs
page_title: Automating Execution of Derrick
description: |-
  An overview of how to automate the execution of Derrick in remote systems
---

# Automating Execution of Derrick

Your team may want to automate the execution of Derrick as your
...
The best option for automating execution is to use Derrick's native
[Git integration](/derrick/docs/projects/git). This will watch a Git repository and
run a `derrick up` whenever a change is detected. This is the recommended
approach.

You may also choose to run the `derrick` CLI in an environment other
...
in place of that, run Derrick by invoking it in
the existing script or system to gradually adopt it.

~> **The recommended way to run Derrick remotely** is through the
Derrick [Git integration](/derrick/docs/projects/git). We recommend trying that
approach first and only scripting the `derrick` CLI if that approach does
not work for you.

## Environment Variables

In a remote execution environment, as with a local workstation, the Derrick
binary will need access to the [Derrick server](/derrick/docs/server). Locally, it is recommended
that developers create [contexts](/derrick/docs/server#connecting) to store information about the remote
Derrick server. In a remote environment, this may make less sense given the
...
configure the Derrick CLI for communication with the Derrick server.

- `WAYPOINT_SERVER_TOKEN`. Must be set to a Derrick token, created with [`derrick user token`](/derrick/commands/user-token)
- `WAYPOINT_SERVER_ADDR`. The address to the Derrick server gRPC address. This must
...
~> The Derrick server token is sensitive and should be
treated as a password and protected accordingly.

## Init

The [`derrick init` command](/derrick/commands/init) is still required in remote environments, and must be
run in the environment ahead of other Derrick commands such as `up`. This
...
each time. A helpful side-effect of `init` is that it will validate the Derrick
...
If Derrick detects that the terminal does not support interactivity it will output a
...
When automating Derrick, it is important to keep in mind that
...
execution of Derrick, it may be important to consider the effect
this could have while all working out of the [`default` workspace](/derrick/docs/workspaces).
...
In this scenario, it is best to use additional [workspaces](/derrick/docs/workspaces). These
...
```shell-session
$ derrick up -workspace $(git symbolic-ref HEAD)
```
...
```shell-session
$ derrick deploy -label git_sha=$GITHUB_SHA -label build_id=$BUILD_ID -label build_hostname=$(hostname)
```

## Locking

Derrick locks operations based on a tuple of (project, app, workspace). If multiple operations
are created (eg. `derrick build`) they will automatically queue.
