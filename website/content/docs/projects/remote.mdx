...
be executed _remotely_ using [runners](/derrick/docs/runner). This enables
...
- At least one [runner is installed and running](/derrick/docs/runner). Derrick
  automatically installs a runner with `derrick install`.
- Your project is configured with a [data source](/derrick/docs/derrick-hcl/runner)
  so that the runners can clone the project source code.
- Your project has [remote operations enabled](/derrick/docs/derrick-hcl/runner).

Derrick will automatically run your operations remotely if all these conditions are met, and that you have a [runner profile](/derrick/docs/runner/profiles) configured.
