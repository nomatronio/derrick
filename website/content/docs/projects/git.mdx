...
- Poll and automatically trigger a `derrick up` when changes are detected
- Manual [remote operations](/derrick/docs/projects/remote)
...
`derrick.hcl` file, and running a command such as `derrick up`.

## Configuring the Project

The Git repository associated with a project is configured with the
`data_source` stanza in the `runner` stanza in the [derrick.hcl file](/derrick/docs/derrick-hcl/runner).
These settings can also be specified via the CLI or web UI.

**It is recommended that you configure the Git data source using
the CLI or web UI.** This avoids tying a Derrick configuration directly
to a single Git repository and also avoids secrets such as Git authentication
information to be present in the Derrick configuration. Learn more about
[how to configure the Git data source using the CLI or web UI here](/derrick/docs/derrick-hcl/runner#cli-or-ui-configuration).

### Add to an existing Derrick project

If you already have a Derrick project, you can add Git integration just as you
would if you were to start a new one. You'll want to configure Git via the CLI
or Web, or include a runner stanza in your `derrick.hcl`. Also ensure your Derrick
server is [at least 0.3.0 and has runners enabled.](#Derrick-Server-Requirement)

#### Configuring git via the UI

If you wish to enable Git settings on a project in the UI, you'll want to navigate
to the project homepage after running `derrick ui -authenticate`. From there,
...
[derrick project apply](/derrick/commands/project-apply#derrick-project-apply)
...
derrick project apply \
   ...
   -derrick-hcl=derrick.hcl \
   my-project
```

#### Configuring via derrick.hcl
...
runner {
  ...
  data_source "git" {
    url  = "https://github.com/nomatronio/derrick-examples.git"
    path = "docker/nodejs"
  }
}
...
automatically trigger a `derrick up` on any changes. To enable polling,
...
on the [`derrick project apply`](/derrick/commands/project-apply#poll) command.
...
Derrick will watch only that ref for changes.

## Debugging Git Integration Failures

For triaging any failures with Git or Derrick runners, you'll want to look in
a couple of places.
...
information or context here, you'll want to dig into the Derrick runner logs
to determine what went wrong.

### Server side Troubleshooting

Derrick uses runners to execute Git operations, build containers, and more. If
...
If you are deploying to Kubernetes, you'll want to look at the Derrick runner
pod logs:

```
kubectl logs pod/derrick-runner-7744b58c9b-mvrcb
```

#### More Information

For more information on debugging and troubleshooting Derrick, please check out
the [docs for troubleshooting](/derrick/docs/resources/troubleshooting).
