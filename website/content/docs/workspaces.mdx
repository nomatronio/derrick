...
specifying the `-workspace` flag to any operation, Derrick will operate
...
`derrick destroy` will destroy all resources within the current workspace.
...
be deleted outside of Derrick if desired.
