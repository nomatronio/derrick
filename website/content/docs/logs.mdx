...
page_title: Logs
description: |-
  Derrick app logs are aggregated and available to view via the CLI and UI.
---
...
Derrick logs provide a real-time snapshot
of logs with minimal retention. The point of Derrick logs are to debug
emergent application behavior rather than historical behavior.
Derrick log aggregation is not meant to be a replacement for a full
logging solution such as Datadog. For most deployments, Derrick will retain
...
full. The Derrick server stores all logs in-memory so this can be used to
predict memory requirements.

-> This is not currently configurable. In a future version of Derrick
...
To view logs with the CLI, use the `derrick logs` CLI. This will
