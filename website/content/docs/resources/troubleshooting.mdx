...
description: |-
  Derrick is a work in progress, and this page provides tips for how to troubleshoot any issues that you might encounter while deploying applications.
---

# Tips and Troubleshooting

## Remove the Derrick Server

The Derrick Server creates several resources in Docker and Kubernetes that should be removed to either reinstall Derrick or to completely remove it from a system.

Run `derrick destroy` on your workspaces prior to removing the server to prevent hanging resources.
...
For Docker, `uninstall` removes the Derrick server-specific volumes, but refrains from
doing a global volume prune. If you created deployments with Derrick, there will likely
...
#### Derrick Server in Docker

Ensure that you have the latest `nomatronio/derrick` Docker image.

```shell-session
$ docker pull nomatronio/derrick:latest
```

[`derrick install`] for Docker creates a container and a volume. These resources should be removed when Derrick Server is no longer needed. These are some example `docker` commands that should clean up after a Derrick Server installation.

```shell-session
$ docker stop derrick-server
$ docker rm derrick-server
$ docker volume prune -f
```

#### Derrick Server in Kubernetes

[`derrick install`] for Kubernetes installs the Derrick Helm chart to your Kubernetes cluster.
...
[`derrick install`]: /derrick/commands/install

```shell-session
$ helm uninstall derrick
$ helm uninstall derrick-static
$ kubectl delete pvc data-derrick-server-0
$ kubectl delete pvc data-default-derrick-static-runner-0
```
...
If your Derrick operation fails, task introspection can help
...
In the example below, after a build failure, `derrick task inspect` was used
...
```terminal-output
derrick task inspect 01G669RE32WM0JBBGZW3JETYSJ
...
» Run Job Configuration
  ...
  Error Messsage: There was an error while executing a Derrick plugin for this operation!
...
Plugin function: github.com/nomatronio/derrick/builtin/pack.(*Builder).BuildODR-fm
...
Derrick includes several commands to support debugging and monitoring while
developing your application.

## Exec Into the Application Container

After deploying your application, you can use `derrick exec` to run
commands in the context of the most recent deployment. Typically, `derrick exec` will be used for running database migrations and debugging. However, you
can use it for any purpose.

Use the `exec` command to open a shell prompt.

```shell-session
$ derrick exec /bin/bash
```

Since you are in the deployment directory, you will observe that Derrick
...
## View Derrick Application and Entrypoint Logs
...
[entrypoint](/derrick/docs/entrypoint) if it is in use.
...
[entrypoint logs](/derrick/docs/entrypoint#logging) for more information.

## Access the Derrick web UI

The Derrick server includes a web-based user interface that you can use to view
...
authenticate to the Derrick web UI.
...
Visit the Derrick UI at port `9702` on the URL of the machine where you are
running the Derrick server. Paste the token into the authentication field.

</Tab>
</Tabs>

~> **NOTE**: Derrick currently uses self-signed certificates for TLS. Your web
