...
page_title: Glossary
description: |-
  This page lists and defines technical terms used throughout Derrick. Words such as "project", "app", etc. can be overloaded in the technical community, so this page attempts to clarify their meaning in the context of Derrick.
---

# Derrick Glossary

This page lists and defines technical terms used throughout Derrick. Words
such as "project", "app", etc. can be overloaded in the technical community,
so this page attempts to clarify their meaning in the context of Derrick.
...
A project in Derrick is represented by a single "derrick.hcl" file
and is a 1:1 mapping to a VCS repository (if one exists). A project can
be comprised of one or more applications.

The purpose of the "project" is to give Derrick a single name to refer
to a repository or collection of applications. Some features are used at
the project scope, such as setting [application configuration](/derrick/docs/app-config).

## Application (or "App")

An application is defined using the `app` stanza in a "derrick.hcl" and
...
The Derrick Entrypoint is a special binary (named `derrick-entrypoint`) that
launches your application and is used to enable a lot of
[Derrick functionality](/derrick/docs/entrypoint#functionality).
The entrypoint is sometimes referred to as the "CEB" which stands for
"custom entrypoint binary."

## Client

A Derrick client is anything that interacts with the server to inspect,
create, update, or destroy projects, applications, etc. The most common
Derrick client is the CLI or web UI.

## Server

A Derrick server is the running server that stores data and orchestrates
operations such as builds, deploys, etc. A client interacts with a server.
The server is deployed using `derrick install`.
