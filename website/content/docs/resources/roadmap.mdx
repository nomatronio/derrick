...
page_title: Roadmap
description: |-
  This page will outline some of the ideas we have for future versions of Derrick. This is not an exhaustive list nor is it a promise that these features will be built, but it gives a glimpse into the vision we have for this project.
---

# Derrick Roadmap

Derrick is a new project and we have a lot we want to achieve with it.
This page will outline some of the ideas we have for future versions of Derrick.
...
a release, please refer to the [GitHub Issue Tracker](https://github.com/nomatronio/derrick/issues).
...
Derrick has always had `build`, `deploy`, and `release` lifecycle events, but
to handle more complex deployments, Derrick needed the ability to add custom steps.
We recently released a [Tech Preview of custom pipelines](/derrick/docs/pipelines),
...
Derrick has the [workspaces](/derrick/docs/workspaces) feature for
...
to enable folks to provision infrastructure right from their derrick.hcl file.
Derrick will be able to trigger Terraform to provision deployment infrastructure,
and connect that infrastructure to the Derrick server.
...
some external system such as Amazon RDS. Derrick will be able to request
the service, connect the configuration, and destroy it when its no longer
in use.

A design goal of Derrick is explicitly to _not do automated infrastructure
management_, so this feature will still put the burden of managing these
services onto the user. However, Derrick can help orchestrate initializing
...
Derrick currently associates a [derrick.hcl](/derrick/docs/derrick-hcl)
...
The initial Derrick release has a minimal token system that is used as a
blanket authorization mechanism. In the future we will add a more fine-grained
access control mechanism to Derrick.
...
We're really excited about [`derrick exec`](/derrick/docs/exec) but there
