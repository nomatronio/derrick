...
description: |-
  Derrick has many different components that interact with each other. This page documents all of these components: what they do, how they work with each other, when they're required, etc.
---

# Derrick Architecture

Derrick has many different components that interact with each other.
This page documents all of these components: what they do, how they
work with each other, when they're required, etc.

## High-Level Overview

The diagram below shows a typical single-platform deployment of Derrick.

![Architecture Overview](/img/architecture.png)

-> Note: some of the components are optional by sacrificing functionality.
It is possible to operate Derrick without entrypoints, but you'll lose
all [entrypoint-required functionality](/waypoint/docs/entrypoint#functionality).

The primary components of Derrick are:

- **Server** - The Derrick server is a long-running, central service that
  serves the Derrick API. This API is consumed by the CLI, entrypoint, and
  other consumers. This is the only component in Derrick that stores state.
  Learn more about [servers here](/waypoint/docs/server).

- **CLI** - The `derrick` CLI acts as a client of the server API. It
  does not store any state locally except for connection information to the
  server.

- **Entrypoint** - The [Derrick Entrypoint](/waypoint/docs/entrypoint) is built-in to
...
- **Restarting deployments with an entrypoint** _may not_ be affected.
  ...
  start the subprocess. If you use [application configuration](/waypoint/docs/app-config),
...
entrypoint to exit. Errors such as failure to connect to a Derrick server
...
See [CLI acts as a runner](/waypoint/docs/resources/internals/execution) for more details.
