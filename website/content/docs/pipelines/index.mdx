...
description: |-
  The overview of Derrick Pipelines.
---

# Derrick Custom Pipelines

~> **NOTE**: Custom Pipelines is currently a **Tech Preview** feature of Derrick.
We invite all Derrick users to give custom pipelines a try and let us know about your experience on our [Discuss forums](https://github.com/nomatronio/derrick/discussions51), as well as feature requests, enhancements, or bugs
via [GitHub](https://github.com/nomatronio/derrick/issues). We look forward to
hearing what you think and how Derrick can help make your experience
delivering your applications even better!

Before the introduction of Pipelines, Derrick had a relatively fixed
built-in pipeline with the `derrick up` CLI command.
An "Up" is a special operation that encapsulates the Build, Deploy, and
Release workflow for an application. This
simple yet powerful pipeline is great for most early users of Derrick to continuously build
and deploy an application to production.

As our product grew, so did our users. Users wished for more fine-tuned control and customization over how Derrick deploys applications. Teams need to run tests after a build, security scan artifacts prior to a deploy, and so on. Custom Pipelines enable teams to solve increasingly complex challenges.
...
Pipelines allow users to tell Derrick what order these operations should
...
Please visit the [Pipeline Configuration Docs](/derrick/docs/pipelines/config)
for more information about the configuration of a Pipeline for a
Derrick project.

### Pipeline Runs

Each time a pipeline is run, Derrick keeps track of the steps, jobs, and run results.
As you can see from the [derrick pipeline run console example output](/derrick/docs/pipelines#examples),
Derrick identifies how many steps are in the pipeline and how many are successfully
completed. Users can then further deep dive into specific pipeline runs to view
more information.

`derrick pipeline list` shows the current steps, last run start and complete
times, last run state, and total runs for every pipeline in the project.

```shell
❯ derrick pipeline list

» Derrick Pipelines for my-project
...
`derrick pipeline inspect` can look up a specific pipeline by ID (default) or
...
```shell
❯ derrick pipeline inspect -name=test
...
```shell
❯ derrick pipeline inspect -name=test -run=3
...
information. See `derrick artifact list-builds` below for an example:

```
❯ derrick artifact list-builds
...
pipeline "example" {
  step "run-some-code" {
    image_url = "https://localhost:5000/derrick-odr:dev"
...
```shell
$ derrick pipeline run -workspace=dev builder
```
...
simple as `derrick pipeline run full-up`:

```shell
❯ derrick pipeline run full-up
