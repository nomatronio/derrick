...
description: |-
  The page for detailing Derrick pipeline configurations
---
...
```hcl
step "my-step-name" {
  image_url = "https://example.com:5000/derrick-odr:latest"
...
Derrick job system. See below for examples of each of these built-in Steps and their
parameters.

### `build`

This built-in Step will run a Build operation job, similar to running `derrick build`.
...
This built-in Step will run a Deploy operation job, similar to running `derrick deploy`.
...
This built-in Step will run a Release operation job, similar to running `derrick release`.
...
This built-in Step will run an Up operation job, similar to running `derrick up`.
...
Derrick does not support running across Project pipelines**.
